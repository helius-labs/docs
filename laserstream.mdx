...
description: Highly configurable low-latency data streams for Solana.
---
...
**Built on Open Standards**: LaserStream is built using **gRPC (Google Remote Procedure Call)**, an open source high-performance RPC framework originally developed by Google and now maintained by the Cloud Native Computing Foundation (CNCF). This means there's no vendor lock-in - you can easily migrate to or from any gRPC-compatible service, and the protocol specifications are fully open and standardized.

Similar to our gRPC offerings, LaserStream nodes tap directly into Solana leaders to receive shreds as they're produced, delivering ultra-low latency data to your application.
...
<Note>
**Drop-in Replacement**: LaserStream is designed as a seamless drop-in replacement for your existing gRPC setup. Since LaserStream uses the industry-standard gRPC protocol (an open source framework), it's fully compatible with existing Yellowstone gRPC implementations. Simply change your endpoint and API token, and your application will work with LaserStream immediately - no proprietary protocols or vendor lock-in.
</Note>
...
LaserStream is designed with multi-protocol support in mind, all built on open standards:

| Protocol | Status | Best For | Features | Implementation |
| --- | --- | --- | --- | --- |
| **gRPC** | ✅ Available | Backend services, high-throughput applications | Highest performance, binary protocol, bi-directional streaming. Uses open source gRPC standard (no vendor lock-in) | Via LaserStream SDK or Yellowstone |
