...
<Tip>
**Try `getTransactionsForAddress`**: Our new [`getTransactionsForAddress`](https://www.helius.dev/docs/rpc/gettransactionsforaddress) method combines `getSignaturesForAddress` and `getTransaction` into a single call, and includes powerful new features including reverse search, time-based filters, and pagination.
</Tip>
