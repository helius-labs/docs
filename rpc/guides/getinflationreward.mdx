...
2.  **`config`** (object, optional): A configuration object with the following optional fields:
    *   **`commitment`** (string, optional): Specifies the [commitment level](https://solana.com/docs/rpc#configuring-state-commitment). Defaults to `finalized` if not provided.
