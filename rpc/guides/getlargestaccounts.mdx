---
title: "How to Use getLargestAccounts to Find Top Accounts by Balance"
description: "Learn to use the Solana getLargestAccounts RPC method to retrieve a list of the top 20 accounts by lamport balance, with options to filter by circulating supply status."
---

# How to Use `getLargestAccounts` to Find Top Accounts by Balance

The `getLargestAccounts` RPC method returns a list of the top 20 accounts on the Solana network, ranked by their lamport balance. This method can be useful for network analysis, understanding wealth distribution, or identifying significant holders of SOL.
...
*   **`commitment`** (string, optional): Specifies the [commitment level](https://solana.com/docs/rpc#configuring-state-commitment) to use when querying the ledger. If not provided, the default commitment of the node is used.
