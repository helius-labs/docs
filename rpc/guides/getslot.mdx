---
title: "How to Use getSlot to Check Current Network Slot"
description: "Learn to use the Solana getSlot RPC method to retrieve the current slot number at a specified commitment level."
---

# How to Use `getSlot`

The `getSlot` RPC method returns the current slot that the RPC node considers to have reached a specific commitment level. This is a fundamental method for understanding the current state of the blockchain as perceived by the node.
...
1.  **`options`** (`object`, optional): An optional configuration object with the following fields:
    *   **`commitment`** (`string`, optional): Specifies the [commitment level](https://solana.com/docs/rpc#configuring-state-commitment) for the query. Supported values are `finalized`, `confirmed`, or `processed`. If omitted, the default commitment of the RPC node is used (usually `finalized`).
...
*   **Commitment Matters:** The returned slot number heavily depends on the `commitment` level specified. `processed` will be the highest (most recent) slot the node is aware of, while `finalized` will be an older slot that is confirmed by the entire network. Choose the commitment level appropriate for your use case's need for data finality.
