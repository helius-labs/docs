---
title: "How to Efficiently Fetch Multiple Accounts with getMultipleAccounts"
description: "Learn to use the Solana getMultipleAccounts RPC method to retrieve data for several accounts in a single request, improving dApp performance."
---
...
2.  **`options`** (`object`, optional): A configuration object containing one or more of the following fields:
    *   **`commitment`** (`string`): Specifies the [commitment level](https://solana.com/docs/rpc#configuring-state-commitment) for the query (e.g., `"finalized"`, `"confirmed"`, `"processed"`).
