...
1.  **`config`** (object, optional): Configuration object containing the following field:
    *   **`commitment`** (string, optional): Specifies the [commitment level](https://solana.com/docs/rpc#configuring-state-commitment) to use when querying the value. If omitted, the default commitment of the RPC node is used (usually `finalized`).
...
*   **Dynamic Value:** The minimum delegation amount is subject to change by network parameters. It's advisable to query this value periodically or before critical operations rather than hardcoding it. You can learn more about staking in our [staking guide](https://www.helius.dev/blog/how-to-stake-solana).
