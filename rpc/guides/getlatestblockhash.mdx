---
title: "How to Use getLatestBlockhash for Transaction Preparation"
description: "Learn to use the Solana getLatestBlockhash RPC method to fetch the latest blockhash and its validity period, crucial for constructing and sending transactions."
---

# How to Use `getLatestBlockhash` for Transaction Preparation

The `getLatestBlockhash` RPC method is essential for preparing and sending transactions on the Solana network. It retrieves the most recent blockhash processed by the node, along with the last block height at which this blockhash will remain valid.
...
*   **`commitment`** (string, optional): Specifies the [commitment level](https://solana.com/docs/rpc#configuring-state-commitment) for the query. If omitted, the node's default commitment is used. For transactions, it's common to use `confirmed` or `finalized` to ensure the blockhash is from a reasonably stable part of the chain.
*   **`minContextSlot`** (integer, optional): The minimum [slot](https://solana.com/docs/terminology#slot) that the request can be evaluated at. This ensures the query is made against a ledger state that has processed up to at least this slot.
...
*   **Choosing Commitment:** For critical transactions, using `finalized` commitment provides the strongest guarantee that the blockhash is from the main chain fork, but it may be slightly older. `confirmed` offers a good balance.
