...
<CardGroup cols={2}>
  <Card title="Subscription Management" icon="list">
    - Limit active subscriptions (max 100-200 per connection)
    - Clean up unused subscriptions promptly
    - Use filters to reduce unnecessary notifications
    - Group related subscriptions on the same connection
  </Card>
  
  <Card title="Reconnection Strategy" icon="rotate">
    - Implement exponential backoff with jitter
    - Set maximum reconnection attempts
    - Maintain subscription state across reconnections
    - Provide fallback to HTTP polling if needed
  </Card>
  
  <Card title="Error Handling" icon="shield-check">
    - Handle all WebSocket error types gracefully
