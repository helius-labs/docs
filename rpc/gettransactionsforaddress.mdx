...
<Tip>
**Helius Exclusive Feature** - `getTransactionsForAddress` is only available through Helius RPC nodes and is not part of standard Solana RPC.
</Tip>
...
<ParamField body="pagination_token" type="string">
...
<ParamField body="filters.status" type="string">
  ...
  - `all`: Both successful and failed (default)
...
|--------|-------------|--------|
| `status` | Filter transactions by success/failure | `succeeded`, `failed`, or `all` |
...
When you have more transactions than your limit, use the `pagination_token` from the response to fetch the next page. The token is a simple string in the format `"slot:position"` that tells the API where to continue from.
...
<CardGroup cols={2}>
  ...
  <Card title="Simpler pagination" icon="list">
    Use `pagination_token` instead of confusing `before`/`until` signatures
  </Card>
</CardGroup>
