...
The method supports advanced filtering by time ranges, slots, transaction status, and signatures. You can sort transactions chronologically or in reverse order, use efficient keyset-based pagination, and optionally retrieve complete transaction data without additional API calls.
...
<Tabs>
  <Tab title="Enhanced Capabilities">
    ...
    - **Advanced filtering**: Time ranges, slot ranges, transaction status
    ...
    - **Status filtering**: Skip failed transactions automatically
  </Tab>
...
<Tabs>
  <Tab title="First Token Mints">
    **Track the earliest mint transactions for a token program:**
    ...
    ```javascript
    {
      ...
      "params": [
        "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA", // Token Program
        {
          ...
          "limit": 50,
          "filters": {
            "status": "succeeded",  // Only successful mints
          }
        }
      ]
    }
    ...
    <Note>
    **Use Case**: Identify the first successful token mints on Solana to analyze early token projects and their creators.
    </Note>
  </Tab>
        ...
        "AMM_PROGRAM_ADDRESS", // e.g., Raydium, Orca
        {
          ...
          "filters": {
            ...
            "blockTime": {
              ...
              "lte": 1640995260   // 1-minute window
            }
          },
  ...
  <Tab title="Time-Based Analytics">
    **Generate monthly transaction reports:**
    ...
    ```javascript
    // Get all successful transactions for January 2024
    const startTime = Math.floor(new Date('2024-01-01').getTime() / 1000);
    const endTime = Math.floor(new Date('2024-02-01').getTime() / 1000);
...
<ParamField body="transactionDetails" type="string" default="signatures">
  ...
  - `full`: Complete transaction data (eliminates need for getTransaction calls)
</ParamField>
...
<Tabs>
  <Tab title="Performance">
    For optimal performance, use `transactionDetails: "signatures"` when you don't need full transaction data. Implement reasonable page sizes between 100-1000 for better response times, and use status filtering to skip failed transactions when building user-facing features.
  </Tab>
    ...
    // Historical replay from the beginning
    {
      ...
      "params": [
        address,
        {
          ...
          "filters": {
            "status": "succeeded",     // Not possible 
            "blockTime": {             // Not possible
              "gte": 1640995200,
              "lte": 1641081600
            }
          }
