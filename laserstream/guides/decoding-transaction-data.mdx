...
**Laserstream sends each transaction in two tidy halves: the *message* user signed (user proposal) and the *meta* the validator wrote back (its verdict).
When you print the update raw those halves are buried in byte arrays, so the first job is simply to see them. We'll start a live subscription, stare at the untouched bytes, add decoders, and then walk the payload twice, first as the user, then as the validator; letting each field appear only when its neighbour needs it.**
  ...
  const request = {
    transactions: {
      "Jupiter-transactions": {
        vote: false,
        failed: false,
        accountsInclude: ['JUP4Fb2cqiRUcaTHdrPC8h2gNsA2ETXiPDD33WcGuJB']
      }
    },
...
When you run the script with the decoding function, you'll see output like this:
...
```json
{
  ...
  "accountKeys": [
    ...
    "J9EYHkAZKU8SsfJvt4pVYH4iCcfo4ECdXVkMtBfwZrEr",
    "4mE1fyipHpVtych6rSNv2kudozMbJRWVmY9LUsMFW",
    "6b1apH6Qx2hR4QbSieprXJ3YVaY4FaTA3ZBMziwVps4J",
    "3b9FpAVtV5mvzHnwYujVpSxc7SMmB6tYgiSi4n1yiEMf",
    "7bRK5tmG1EKxsfWfr7x4R2GBK9Q75PP3eVXN9YozcXQT",
    "BqbP7EUdgaJPseNkLKZuziqKWvMiTrrgKtcWVgouGQiK",
    "CvDL7FRf87wKBFPx9RqtgTEfKiDgricAidH983bDj1a",
    ...
    "A8C2M8HNJP1nDaTLuQgeXayTBmoAztMkGYrwiWhpCAii",
    "ADyA8hdefvWN2dbGGWFotbzWxrAvLW83WG6QCVXvJKqw",
    "DQGTVB1939V9VPMAwYtJrN8HXmwfg3KvnNUM3iw7dJDp",
...
## Built-in decoding utilities

Laserstream uses the standard Yellowstone gRPC protocol, which provides built-in transaction decoding utilities. Instead of manually implementing three separate decoders, we can use a single recursive function to convert all binary fields to human-readable format.
...
`accountKeys` is a plain list of public keys. Every later integer - `signatures[i]`, `programIdIndex`, each element in an instruction's `accounts` array - points back into this list, saving more than a kilobyte per message.
...
* who must sign
* which program and accounts the call touches
* the deadline for inclusion
* any bonus keys fetched from tables
...
```json
{
  "logMessages": [
    ...
    "Program JUP4Fb2cqiRUcaTHdrPC8h2gNsA2ETXiPDD33WcGuJB consumed 23432 of 200000 compute units",
...
`preTokenBalances` and `postTokenBalances` list SPL-Token accounts with a ready-made `uiTokenAmount` so you avoid dividing by 10⁹ yourself.
