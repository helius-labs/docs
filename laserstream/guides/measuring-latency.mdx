...
**⚠️ CRITICAL DISCLAIMER: Production Data Center Only**
...
**⚠️ CO-LOCATION REQUIREMENT: Deploy Near Your LaserStream Endpoint**
...
The most reliable method compares two independent streams to the same data source, measuring which receives identical events first.

### 2. Local timestamp vs created_at comparison

Measures the difference between when your system receives a message and the timestamp embedded in the message by the LaserStream service.

**Important caveat:** This measurement has limitations since `created_at` only represents when the LaserStream node created the message internally. If LaserStream received data late for some reason, that upstream delay won't be captured by `created_at`, making this less accurate than Method 1 for measuring true end-to-end latency.

### 3. Block timestamp analysis (not recommended)

Compares local receipt time against Solana's block timestamp, but this method has significant limitations due to block timestamps only having second-level granularity while Solana produces blocks every 400ms.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Subscription established; waiting for messages …");
    ...
    while let Some(update_res) = subscribe_rx.next().await {
        ...
        if let Some(ts) = update.created_at {
            ...
            let latency_ms = calculate_latency_ms(&ts);

            let local_ms = system_time_to_millis(now);
            let created_ms = timestamp_to_millis(&ts);

            debug!(local_ms, created_ms, latency_ms, commitment_level = COMMITMENT_LEVEL, "Message timing");
            println!(
                "commitment_level={}, local_ms={}, created_ms={}, latency_ms={}",
                COMMITMENT_LEVEL, local_ms, created_ms, latency_ms
            );
        } else {
...
fn calculate_latency_ms(created_at: &Timestamp) -> i128 {
    let now = SystemTime::now();
    let created_at_systime = SystemTime::UNIX_EPOCH
        + Duration::from_secs(created_at.seconds as u64)
        + Duration::from_nanos(created_at.nanos as u64);

    if now > created_at_systime {
        now.duration_since(created_at_systime)
            .unwrap()
            .as_millis() as i128
    } else {
        -(created_at_systime
...
fn timestamp_to_millis(ts: &Timestamp) -> i128 {
    (ts.seconds as i128 * 1000) + (ts.nanos as i128 / 1_000_000)
}
```

**What this measures:** End-to-end latency from when the LaserStream service creates a message to when your application receives it.

**Running the test:**
```bash
cargo run --bin timestamp-latency
```

**Sample output:**
``` json [expandable]
commitment_level=processed, local_ms=1752168998488, created_ms=1752168998290, latency_ms=3
commitment_level=processed, local_ms=1752168998572, created_ms=1752168998392, latency_ms=2
commitment_level=processed, local_ms=1752168998695, created_ms=1752168998523, latency_ms=4
commitment_level=processed, local_ms=1752168998744, created_ms=1752168998571, latency_ms=1
commitment_level=processed, local_ms=1752168998990, created_ms=1752168998825, latency_ms=5
commitment_level=processed, local_ms=1752168999294, created_ms=1752168999091, latency_ms=2
commitment_level=processed, local_ms=1752168999391, created_ms=1752168999220, latency_ms=3
commitment_level=processed, local_ms=1752168999827, created_ms=1752168999650, latency_ms=4
commitment_level=processed, local_ms=1752168999876, created_ms=1752168999700, latency_ms=0
commitment_level=processed, local_ms=1752169000441, created_ms=1752169000219, latency_ms=2
commitment_level=processed, local_ms=1752169000881, created_ms=1752169000226, latency_ms=5
commitment_level=processed, local_ms=1752169000881, created_ms=1752169000227, latency_ms=4
commitment_level=processed, local_ms=1752169000882, created_ms=1752169000481, latency_ms=1
commitment_level=processed, local_ms=1752169000882, created_ms=1752169000481, latency_ms=1
commitment_level=processed, local_ms=1752169000882, created_ms=1752169000500, latency_ms=3
commitment_level=processed, local_ms=1752169001143, created_ms=1752169000940, latency_ms=2
commitment_level=processed, local_ms=1752169001270, created_ms=1752169001106, latency_ms=4
commitment_level=processed, local_ms=1752169002119, created_ms=1752169001911, latency_ms=3
commitment_level=processed, local_ms=1752169002210, created_ms=1752169002030, latency_ms=2
commitment_level=processed, local_ms=1752169002574, created_ms=1752169002390, latency_ms=5
commitment_level=processed, local_ms=1752169002620, created_ms=1752169002436, latency_ms=4
commitment_level=processed, local_ms=1752169002624, created_ms=1752169002454, latency_ms=1
commitment_level=processed, local_ms=1752169003283, created_ms=1752169003112, latency_ms=3
commitment_level=processed, local_ms=1752169003605, created_ms=1752169003434, latency_ms=2
commitment_level=processed, local_ms=1752169003718, created_ms=1752169003537, latency_ms=4
commitment_level=processed, local_ms=1752169003790, created_ms=1752169003627, latency_ms=0
```

This outputs the latency in milliseconds for each received message, showing the distribution of end-to-end delivery times.

---

## Method 3: Block timestamp comparison (not recommended)

While it might seem logical to compare block timestamps against your local receipt time, this approach fundamentally fails for latency measurement due to a critical mismatch in precision. Solana produces blocks approximately every 400 milliseconds, creating roughly 2.5 blocks per second, but block timestamps only record time at second-level granularity.

This creates an impossible situation: when multiple blocks are produced within the same second, they all appear to have identical timestamps despite being created hundreds of milliseconds apart. The precision you need for meaningful latency measurement simply doesn't exist in the block timestamp data.

Even if you could work around the granularity issue, block timestamps reflect when validators agreed on block finalization, not when the underlying transaction data first became available. This adds another layer of uncertainty that makes the measurement less useful for understanding streaming latency.

The fundamental problem is that you're trying to measure sub-second performance differences using timestamps that only change every second. This approach should be avoided entirely in favor of the previous two methods that provide the precision needed for actual latency analysis.

---

## Higher-level intuition

**Think of latency measurement as answering three fundamental questions:**

1. **"Who's faster?"** - Method 1 (parallel comparison) gives you the clearest answer by eliminating variables and directly comparing two streams receiving identical data.

2. **"How fast am I getting data?"** - Method 2 (created_at analysis) shows your application's end-to-end experience, though it has limitations since it only measures from when LaserStream packaged the message.

3. **"What should I avoid?"** - Method 3 (block timestamps) demonstrates why some approaches fail - blockchain timestamps lack the precision needed for meaningful latency measurement.

**The key insight:** In distributed systems like blockchain networks, true latency measurement requires understanding what you can and cannot control. You can't measure absolute speed to a distributed network, but you can measure relative performance between different paths to the same data.

**Practical takeaway:** Use Method 1 for infrastructure decisions, Method 2 for application monitoring, and deploy your testing infrastructure as close as possible to your chosen LaserStream region. Remember that latency measurement in production data centers reveals meaningful differences that local testing simply cannot capture. 