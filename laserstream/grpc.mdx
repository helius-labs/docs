...
<Tip>
**Region Selection**: For best performance, choose the region closest to your server. For example, if deploying in Europe, use either the Amsterdam (`ams`) or Frankfurt (`fra`) endpoint.
</Tip>
...
<Steps>
  ...
  <Step title="Replace Your API Key and Choose Your Region">
    ...
    const config: LaserstreamConfig = {
      ...
      endpoint: 'https://laserstream-mainnet-fra.helius-rpc.com', // Example: Frankfurt region
    }
    ```

    **Region Selection Examples:**
    - If your server is in Europe: Use `fra` (Frankfurt) or `ams` (Amsterdam)
    - If your server is in US East: Use `ewr` (New York)
    - If your server is in US West: Use `slc` (Salt Lake City)
    - If your server is in Asia: Use `tyo` (Tokyo) or `sgp` (Singapore)
  </Step>
