---
title: "How to Use getBlockProduction to Monitor Block Production"
description: "Learn to use the Solana getBlockProduction RPC method to retrieve information about block production by validators."
---

# How to Use `getBlockProduction` to Monitor Block Production
...
1.  **`commitment`** (string, optional): Specifies the [commitment level](https://docs.solana.com/developing/clients/jsonrpc-api#configuring-state-commitment) to use for the query. If omitted, the default commitment of the node is used.
...
*   **Node Data Availability:** The range of historical data available can vary between RPC nodes. Very old slot ranges might not be available.
*   **Epoch Boundaries:** When querying by `identity` without a `range`, the data returned is for the current epoch up to the latest processed slot by the node.
*   **Mainnet-beta vs. Devnet/Testnet:** The examples use `https://mainnet.helius-rpc.com/?api-key=<api-key>`. Adjust the RPC endpoint for Devnet or Testnet as needed. Validator identities and slot numbers will also differ.
