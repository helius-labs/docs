---
title: "How to Use getLeaderSchedule to Find Epoch Leaders"
description: "Learn to use the Solana getLeaderSchedule RPC method to retrieve the leader schedule for an epoch, optionally filtering by validator identity."
---

# How to Use `getLeaderSchedule` to Find Epoch Leaders

The `getLeaderSchedule` RPC method returns the assignment of block production leadership to validators for a specific epoch. Understanding the leader schedule can be useful for network analysis, predicting which validator will produce blocks at certain times, or for tools that interact with specific leaders.
...
2.  **`config`** (object, optional): A configuration object that can contain:
    *   **`commitment`** (string, optional): Specifies the [commitment level](https://solana.com/docs/rpc#configuring-state-commitment). If not provided, the node's default commitment is used.
