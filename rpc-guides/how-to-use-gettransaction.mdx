---
title: "How to Use getTransaction to Retrieve Transaction Details"
description: "Learn to use the Solana getTransaction RPC method to fetch detailed information about a confirmed transaction using its signature."
---

# How to Use `getTransaction`

The `getTransaction` RPC method allows you to retrieve detailed information about a confirmed transaction by providing its signature. This includes the transaction's slot, block time, metadata (like fees, status, and balance changes), and the transaction structure itself.
...
2.  **`options`** (object, optional): An optional configuration object that can include:
    *   **`commitment`** (string, optional): Specifies the [commitment level](https://docs.solana.com/developing/clients/jsonrpc-api#configuring-state-commitment) (e.g., `"finalized"`, `"confirmed"`). If not provided, the node's default commitment is used (usually `"finalized"`).
