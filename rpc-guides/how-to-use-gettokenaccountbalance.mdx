---
title: "How to Use getTokenAccountBalance to Check SPL Token Balances"
description: "Learn to use the Solana getTokenAccountBalance RPC method to retrieve the balance of a specific SPL Token account."
---
...
2.  **Configuration Object** (object, optional): An optional object that can contain the following field:
    *   **`commitment`** (string, optional): Specifies the [commitment level](https://docs.solana.com/developing/clients/jsonrpc-api#configuring-state-commitment) for the query. If omitted, the default commitment of the RPC node is used (usually `finalized`).
