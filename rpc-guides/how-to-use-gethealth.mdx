---
title: "How to Use getHealth to Check Node Status"
description: "Learn to use the Solana getHealth RPC-related endpoint to check if a node is healthy and synchronized with the cluster."
---

# How to Use `getHealth` to Check Node Status
...
Unlike most other RPC methods that use a JSON-RPC POST request, the `getHealth` check is often implemented as a simple HTTP `GET` request to the `/health` path of the RPC endpoint (e.g., `https://api.mainnet-beta.solana.com/health`).
...
// Example usage:
checkNodeHealth('https://api.mainnet-beta.solana.com');
checkNodeHealth('https://api.devnet.solana.com');
...
// checkRpcHealthViaPost('https://api.mainnet-beta.solana.com');
