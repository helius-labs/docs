---
title: "Solana Webhooks: Real-Time Blockchain Event Notifications"
sidebarTitle: Overview
description: "Monitor Solana blockchain events with real-time webhooks. Get instant notifications for NFT sales, token transfers, and transactions sent to your endpoints."
---
...
For frequently asked questions about webhooks, please refer to our [Webhooks FAQ](/faqs/webhooks).
...
Helius automatically retries webhook pushes when your server fails to send a proper response.

Your endpoint should return a 200 status code to acknowledge successful receipt.

If your endpoint returns an error status code or fails to respond at all, we will retry the delivery up to 3 times with a 1-minute interval between attempts.

If all retry attempts fail, the webhook event will be dropped.
...
To handle duplicates, use the transaction signature or a unique event identifier to track which events you've already processed. Store processed signatures in a database or cache, and skip processing if you encounter a duplicate.
