...
直接查询区块链上的这些数据对于任何实际的交易分析来说都太慢。相反，量化交易员可能会选择为SOL-USDC市场构建索引，并使用实时流产品如[LaserStream](/zh/laserstream)更新最新交易。
...
* 根据需要重复此循环  
* 对于通过这种方式检索到的每个交易签名，调用 `getTransaction` 以获取其完整的交易详情  
* 将相关数据插入到您的数据库中
...
* 为每笔交易进行一次额外的 RPC 调用
* 构建一个线程安全的队列来处理并发处理
* 构建重试和退避逻辑以防止数据丢失和达到速率限制
* 不包括涉及地址拥有的关联代币账户的交易

虽然此方法有效，但它更复杂、灵活性较差，并且消耗更多的[积分](/zh/billing/credits)。要获取包括代币账户在内的完整钱包历史记录，请改用 `getTransactionsForAddress`。

### 方法 3：使用 getBlock

当目标区块中的交易有很大比例与您的分析相关时，例如索引[经常使用的 Solana 程序](/zh/orb/explore-programs)的交易，如 DFlow 的 Aggregator、Pump.fun 程序或 Solana 的 Token 程序，[`getBlock`](/zh/rpc/guides/getblock) 方法最为有效。
...
数据湖非常适合存储大量原始和处理过的区块链数据，用于长期归档和分析查询。
...
有了我们新的[最先进的存档系统](https://www.helius.dev/blog/introducing-gettransactionsforaddress)、如 **getTransactionForAddress** 这样的存档调用，以及像 LaserStream 这样的行业领先的数据流解决方案，构建 Solana 索引比以往更简单、更实用。
