...
公司还利用自定义索引将链下数据（例如，中心化交易所价格，KYC信息等）与他们的链上数据结合起来。
...
直接查询区块链以获取这些数据对于任何实际的交易分析来说都太慢。相反，量化交易员可能会选择为SOL-USDC市场构建索引，并使用实时流产品如[LaserStream](/zh/laserstream)保持其更新以获取最新交易。
...
一个简单的实现使用Parquet数据格式与Amazon Athena。
...
* **可靠性**：没有历史重播保证。如果您的WebSocket断开连接，您需要手动检测并使用RPC方法补充缺口
