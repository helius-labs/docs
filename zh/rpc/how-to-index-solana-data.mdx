...
在此设置中，`(token_mint, date)`被设置为主键和排序键。ClickHouse将根据您的排序键在磁盘上排序数据。这对于查询单个[token mint](/zh/orb/explore-mint-addresses)并通过日期范围缩小响应非常理想。
...
为了优化查询性能，可以使用物化视图预先计算常见的聚合。

例如，您可以预先计算每日的代币转移量，以便在仪表板上的与量相关的图表中使用。

### 选项3：数据湖

数据湖非常适合存储大量的原始和处理过的区块链数据，用于长期归档和分析查询。
...
每天的转移都存储在其对应日期文件夹中的单独Parquet文件中。
