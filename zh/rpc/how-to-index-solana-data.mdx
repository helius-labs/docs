...
当目标区块中的交易有很大比例与您的分析相关时，[`getBlock`](/zh/rpc/guides/getblock) 方法最为有效，例如索引 [常用 Solana 程序](/zh/orb/explore-programs) 的交易，如 DFlow 的 Aggregator、Pump.fun 程序或 Solana 的 Token 程序。
...
建议将Solana数据存储在像PostgreSQL这样的关系型数据库中，适用于大多数用例。SQL灵活、普遍且易于学习。现代关系型数据库可以扩展到超过1亿行，同时仍然为您提供ACID合规性、复杂的连接和强大的二级索引的好处。

使用**SQLite**进行原型设计、本地开发或当您需要零配置的单文件数据库时。它在您的数据集保持在几GB以下时是理想的选择。
...
首先，我们需要将它们存储在 S3 中：创建一个名为 `solana_index` 的存储桶，并使用此键结构按时间对您的代币转移数据进行分区：
...
使用 [**Carbon**](https://github.com/sevenlabs-hq/carbon) 和类似框架，以避免编写样板代码，并在数小时而非数天内设置索引器。
...
通过我们新的[最先进的存档系统](https://www.helius.dev/blog/introducing-gettransactionsforaddress)、如 **getTransactionForAddress** 的存档调用，以及像 LaserStream 这样的行业领先的数据流解决方案，构建 Solana 索引比以往更简单、更实用。
