...
直接查询区块链上的这些数据对于任何实际的交易分析来说都太慢。相反，量化交易员可能会选择为SOL-USDC市场构建索引，并使用实时流产品如[LaserStream](/zh/laserstream)保持其更新以获取最新交易。
...
[`getTransactionsForAddress`](/zh/rpc/gettransactionsforaddress) RPC方法允许您获取区块链数据任意段的完整交易详情。由于其强大的过滤能力，您不会浪费时间检索不需要的数据，并且由于其反向搜索功能，您可以按时间顺序获取交易。
...
使用[gTFA端点](/zh/api-reference/rpc/http/gettransactionsforaddress)的主要优势是速度和简单性。通过槽位和基于时间的过滤器、反向搜索和分页，您可以从Solana历史中的任何时间获取所需的任何数据，只需一次调用，无需复杂的循环或重试逻辑。
...
仅在您正在检查频繁使用的程序的交易或地址过滤无法捕获目标数据时使用此方法。
...
每天的转移都存储在其相应日期文件夹中的单独Parquet文件中。

当您处理Solana的转移时，将其转换为Parquet格式并写入相应的S3对象中。
