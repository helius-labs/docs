...
索引是从Solana区块链查询数据并将其存储在后端数据库（例如，PostgreSQL，ClickHouse）中的过程，然后可以用来随时响应客户请求，而无需使用[Solana RPC调用](/zh/api-reference/rpc/http-methods)直接查询区块链。
...
直接查询区块链上的这些数据对于任何实际的交易分析来说都太慢。相反，量化交易员可能会选择为SOL-USDC市场建立索引，并使用像[LaserStream](/zh/laserstream)这样的实时流产品更新最新交易。
...
首先，我们需要将它们存储在S3中：创建一个名为`solana_index`的存储桶，并使用此键结构按时间对您的代币转移数据进行分区：
...
* 探索 [LaserStream 快速入门指南](/zh/laserstream) 以了解实时流