---
title: WebSocket 快速入门：构建实时 Solana 应用
sidebarTitle: 快速入门
description: 几分钟内开始使用 Solana WebSockets。提供账户监控、交易跟踪和实时应用的实用示例的分步指南。
---
...
<Steps>
  <Step title="获取您的API密钥">
    从[仪表板](https://dashboard.helius.dev)获取您的Helius API密钥。您将需要此密钥进行身份验证。
  </Step>
...
<Card title="完整的 WebSocket API 参考" icon="book" href="/zh/api-reference/rpc/websocket-methods">
...
<CardGroup cols={2}>
  <Card title="WebSocket 概述" icon="book" href="/zh/rpc/websocket">
    了解 WebSocket 概念、承诺级别和高级重连模式
  </Card>
  <Card title="增强型 WebSockets" icon="signal-stream" href="/zh/enhanced-websockets">
    探索具有附加功能和更好过滤的 Helius 增强型 WebSockets
  </Card>
  <Card title="LaserStream" icon="bolt" href="/zh/laserstream">
    升级到 LaserStream 以实现超高速数据流和历史重播
  </Card>
  <Card title="API 参考" icon="code" href="/zh/api-reference/rpc/websocket-methods">
    完整的 WebSocket 订阅方法文档
  </Card>
</CardGroup>

如有疑问或需要额外支持，请加入我们的 [Discord 社区](https://discord.com/invite/6GXdee3gBj)，我们的团队和社区成员随时准备提供帮助！