...
<Tabs>
  ...
  <Tab title="访问控制规则">
    ...
    <Frame caption="在仪表板的 Endpoints 部分配置 RPC 访问控制规则。">
      <img src="/images/rpc-access-control-rules-ip-domain-cidr.webp" alt="在仪表板的 Endpoints 部分配置 RPC 访问控制规则。" />
    </Frame>
  ...
  <Tab title="安全 URL">
    ...
    <Info>
    **速率限制**：安全 URL 限制为每个 IP 地址每秒 5 个事务 (TPS)，非常适合前端应用程序。
    </Info>

    ### 如何使用安全 URL

    <Steps>
      <Step title="获取您的安全 URL">
        在 Helius 仪表板的 Endpoints 部分找到您的安全 URL
      </Step>
      <Step title="替换您的常规 RPC URL">
        使用安全 URL 替换您的常规 RPC URL 和 API 密钥
      </Step>
      <Step title="无需 API 密钥">
        安全 URL 不需要 API 密钥参数
      </Step>
    </Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="支持团队" icon="headset" href="/support">
    联系我们的支持团队以解决安全问题
  </Card>
</CardGroup>
