...
<Note>
字段 `transactionIndex` 是 `getTransactionsForAddress` 独有的。其他类似的端点如 `getSignaturesForAddress`、`getTransaction` 和 `getTransactions` 不包含此字段。
</Note>
...
当交易数量超过限制时，使用响应中的`paginationToken`获取下一页。该令牌是一个简单的字符串，格式为`"slot:position"`，用于告诉 API 从哪里继续。
...
对于较大的历史扫描，通过时间或槽窗口进行迭代（例如，每次 1000 个槽），并重复此模式。

<Note>
每个地址查询都算作一个单独的 API 请求（每个地址 100 个积分）。
</Note>
...
为了获得最佳性能，当不需要完整的交易数据时，使用`transactionDetails: "signatures"`。实现合理的页面大小以获得更好的响应时间，并考虑按时间范围或特定槽进行过滤以进行更有针对性的查询。
...
使用指数退避策略优雅地处理速率限制。在发出请求之前始终验证地址，并在适当时缓存结果，以减少API使用并提高应用程序性能。
