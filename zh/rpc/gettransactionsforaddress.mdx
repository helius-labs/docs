...
<Tip>
**Helius 独有功能** - `getTransactionsForAddress` 仅通过 Helius RPC 节点提供，并不是标准 Solana RPC 的一部分。此端点需要 [开发者计划](/zh/billing/plans-and-rate-limits) 或更高版本。
</Tip>
...
此方法在多个场景中特别有用。**代币发行分析**有助于跟踪新项目的首次铸币交易和早期代币持有者。**钱包资金历史**允许您识别特定地址的资金来源和交易模式。**交易分析**让您可以按成功/失败状态进行过滤，专注于已完成的交易，排除失败的尝试。

该 API 还支持通过状态过滤为特定时间段生成交易报告的 **审计与合规** 工作流程。**分析仪表板**可以利用历史重播功能构建全面的交易分析。最后，**投资组合跟踪** 应用程序可以访问完整的成功交易历史，用于 DeFi 投资组合管理。
...
<Steps>
  <Step title="获取您的 API 密钥">
    从 [Helius 仪表板](https://dashboard.helius.dev/api-keys) 获取您的 API 密钥。
  </Step>
  ...
  <Step title="了解参数">
    ...
    - **sortOrder**: 使用 `'asc'` 进行按时间顺序排序（最早的在前）或 `'desc'` 进行按最新排序
...
使用过滤器时，可以对`slot`, `blockTime`或`signature`使用比较操作符，以及一个特殊的`status`过滤器。这些操作符的工作方式类似于数据库查询，提供对数据范围的精确控制。
...
为了获得最佳性能，当不需要完整交易数据时，使用`transactionDetails: "signatures"`。实现合理的页面大小以获得更好的响应时间，并考虑按时间范围或特定槽位进行过滤以获得更有针对性的查询。

### 过滤

从广泛的过滤器开始，逐步缩小范围以找到您需要的数据。使用基于时间的过滤器进行分析和报告工作流。您可以结合多个过滤器进行精确查询，以定位特定的交易类型或时间段。
...
如果您熟悉标准`getSignaturesForAddress`方法，以下是主要区别：
...
<CardGroup cols={2}>
  ...
  <Card title="Discord社区" icon="discord" href="https://discord.com/invite/6GXdee3gBj">
