...
此方法在多个场景中特别有用。**代币发行分析**帮助跟踪新项目的首次铸币交易和早期代币持有者。**钱包资金历史**允许您识别特定地址的资金来源和交易模式。**交易分析**让您可以按成功/失败状态过滤，以专注于已完成的交易并排除失败的尝试。

该 API 还支持通过状态过滤为特定时间段生成交易报告的 **审计与合规** 工作流程。**分析仪表板**可以利用历史重播功能构建全面的交易分析。最后，**投资组合跟踪** 应用程序可以访问完整的成功交易历史，以进行 DeFi 投资组合管理。
...
<Steps>
  ...
  <Step title="了解参数">
    ...
    - **sortOrder**: 使用 `'asc'` 进行按时间顺序排序（最早的在前）或 `'desc'` 进行最新的在前排序
