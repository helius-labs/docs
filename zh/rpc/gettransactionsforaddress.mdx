...
此方法独特之处在于它允许您查询**完整的代币历史**。您可以查询钱包的完整历史记录，包括关联的代币地址（ATAs）。
本地 RPC 方法如 getSignaturesForAddress 不包括 ATAs。

`tokenAccounts` 过滤器让您可以控制此行为：
- **`none`**（默认）：仅返回直接引用钱包地址的交易。当您只关心直接的钱包交互时使用此选项。
- **`balanceChanged`**（推荐）：返回引用钱包地址或修改钱包拥有的代币账户余额的交易。这可以过滤掉垃圾邮件和无关操作，如费用收取或委托，给您一个清晰的有意义的钱包活动视图。
