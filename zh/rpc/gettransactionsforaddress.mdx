...
<Steps>
  ...
  <Step title="了解参数">
    ...
    - **sortOrder**: 使用 `'asc'` 以时间顺序（最旧的优先）或 `'desc'` 以最新的优先
...
<ParamField body="sortOrder" type="string" default="desc">
  结果的排序顺序：
  - `desc`: 最新的优先（默认）
  - `asc`: 最旧的优先（按时间顺序，非常适合历史分析）
</ParamField>

<ParamField body="limit" type="number" default="1000">
  返回的最大交易数：
  - 当 `transactionDetails: "signatures"` 时最多 1000
  - 当 `transactionDetails: "full"` 时最多 100
</ParamField>
...
使用过滤器时，可以对`slot`, `blockTime`或`signature`使用比较运算符，以及一个特殊的`status`过滤器。这些运算符的工作方式类似于数据库查询，使您能够精确控制数据范围。
...
| `transactionIndex` | number | 交易在其区块中的从零开始的索引。用于交易排序和区块重建。 |
...
为了获得最佳性能，当您不需要完整的交易数据时，使用`transactionDetails: "signatures"`。实现合理的页面大小以获得更好的响应时间，并考虑按时间范围或特定槽进行过滤以获得更有针对性的查询。
...
在需要稍后恢复大型查询时存储分页键。监控分页深度以进行性能规划，并在需要按时间顺序重播历史事件的场景中使用升序。
