...
<Tip>
**Helius 独家功能** - `getTransactionsForAddress` 仅通过 Helius RPC 节点可用，并不是标准 Solana RPC 的一部分。此端点需要 [开发者计划](/zh/billing/plans) 或更高版本，并且每次请求花费 100 积分。返回 100 个完整交易或 1,000 个签名。
</Tip>
...
使用过滤器时，可以对`slot`, `blockTime`, 或 `signature`使用比较运算符，加上一个特殊的`status`过滤器。这些运算符的工作方式类似于数据库查询，提供对数据范围的精确控制。
...
对于较大的历史扫描，遍历时间或槽窗口（例如，每次1000个槽），并重复此模式。
