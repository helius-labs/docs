...
<Tip>
**Helius 独家功能** - `getTransactionsForAddress` 仅通过 Helius RPC 节点提供，并不是标准 Solana RPC 的一部分。此端点需要 [开发者计划](/zh/billing/plans) 或更高版本，并且每次请求花费 100 积分。返回 100 个完整交易或 1,000 个签名。
</Tip>

## 概述

[`getTransactionsForAddress`](/zh/api-reference/rpc/http/gettransactionsforaddress) 提供强大的交易历史查询，具有高级过滤、灵活排序和高效分页功能。

### 主要功能

<CardGroup cols={2}>
  <Card title="灵活排序" icon="arrows-up-down">
    按时间顺序排序（最旧的在前）或反向排序（最新的在前）
  </Card>
...
当有人给您发送 USDC 时，它会进入您的 USDC 代币账户，而不是您的主钱包地址。
</Note>

此方法独特之处在于它允许您查询**完整的代币历史**。您可以查询钱包的完整历史记录，包括关联的代币地址（ATA）。
本地 RPC 方法如 getSignaturesForAddress 不包括 ATA。

`tokenAccounts` 过滤器让您可以控制此行为：
- **`none`**（默认）：仅返回直接引用钱包地址的交易。当您只关心直接的钱包交互时使用此选项。
- **`balanceChanged`**（推荐）：返回引用钱包地址或修改钱包拥有的代币账户余额的交易。这会过滤掉垃圾邮件和不相关的操作，如费用收取或委托，给您一个清晰的有意义的钱包活动视图。
...
<Steps>
  <Step title="获取您的API密钥">
    从[Helius Dashboard](https://dashboard.helius.dev/api-keys)获取您的API密钥。
  </Step>
...
<ParamField body="filters.tokenAccounts" type="string" default="none">
  过滤相关代币账户的交易：
  - `none`：仅返回引用提供的地址的交易（默认）
  - `balanceChanged`：返回引用提供的地址或修改由提供的地址拥有的代币账户余额的交易（推荐）
  - `all`：返回引用提供的地址或由提供的地址拥有的任何代币账户的交易
...
<ParamField body="maxSupportedTransactionVersion" type="number">
  设置要返回的最大交易版本。如果省略，则仅返回传统交易。设置为`0`以包含所有版本的交易。
</ParamField>
...
使用过滤器时，可以对`slot`、`blockTime`或`signature`使用比较运算符，以及特殊的`status`和`tokenAccounts`过滤器。

### 比较运算符

这些运算符的工作方式类似于数据库查询，使您可以精确控制数据范围。
...
| `eq` | 等于 | 包含值完全等于（仅限`blockTime`） | `blockTime: { eq: 1641081600 }` |
...
| `slot` | number | 包含此交易的区块的槽位。 |
| `transactionIndex` | number | 交易在其区块中的从零开始的索引。对交易排序和区块重建有用。 |
| `blockTime` | number \| null | 估计的生产时间，以 Unix 时间戳表示（自纪元以来的秒数）。 |
| `err` | object \| null | 如果交易失败则为错误，成功则为 null。仅在签名模式下。 |
| `memo` | string \| null | 与交易相关的备忘录。仅在签名模式下。 |
...
当交易数量超过限制时，使用响应中的 `paginationToken` 来获取下一页。该令牌是一个简单的字符串，格式为 `"slot:position"`，用于告诉API从哪里继续。
...
为了获得最佳性能，当您不需要完整的交易数据时，请使用`transactionDetails: "signatures"`。实现合理的页面大小以获得更好的响应时间，并考虑按时间范围或特定槽进行过滤以进行更有针对性的查询。
...
使用`getSignaturesForAddress`，您需要先调用`getTokenAccountsByOwner`，然后查询每个代币账户：
...
<CardGroup cols={2}>
  ...
  <Card title="更简单的分页" icon="list">
    使用`paginationToken`代替令人困惑的`before`/`until`签名
  </Card>
</CardGroup>
