...
[`getBalance`](https://www.helius.dev/docs/api-reference/rpc/http/getbalance) RPC 方法是一个简单的方法，用于查询 Solana 区块链上任何账户的原生 SOL 余额。它以 lamports 返回余额（1 SOL = 1,000,000,000 lamports）。
...
*   **使用`getMultipleAccounts`进行批处理：** 虽然`getBalance`用于单个账户，但如果您需要多个账户的余额，使用`getMultipleAccounts`然后从每个账户的信息中提取lamport余额可能比进行多次单独的`getBalance`调用更高效。