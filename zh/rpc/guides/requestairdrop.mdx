...
JSON-RPC 响应中的 `result` 字段是一个表示空投交易签名的字符串，base-58 编码。
...
*   **确认:** 在 `requestAirdrop` 返回签名后，交易仍需由网络处理并确认。您可以使用 `confirmTransaction`（来自 `@solana/web3.js`）或轮询 `getSignatureStatuses` 来等待确认。
