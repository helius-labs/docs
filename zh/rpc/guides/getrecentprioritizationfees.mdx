...
[`getRecentPrioritizationFees`](https://www.helius.dev/docs/api-reference/rpc/http/getrecentprioritizationfees) RPC 方法提供了对 Solana 网络中最近区块中支付的优先费用的洞察。通过检查这些费用，开发者可以更好地决定在交易中附加多少额外费用（[优先费用](https://www.helius.dev/blog/priority-fees-understanding-solanas-transaction-fee-mechanics)），以提高其交易被及时处理的可能性，特别是在网络活动高峰期。
...
1.  **`lockedWritableAccounts`** (`array` of `string`, 可选):
    *   一个 base-58 编码的账户公钥数组，表示您的交易打算写锁的账户。
    *   最多可以提供 128 个地址。
    *   如果提供，该方法返回锁定了**所有**指定账户为可写的交易所支付的优先费用。
    *   如果省略或传递一个空数组，该方法返回最近区块中观察到的优先费用的更一般视图，而不是特定于任何特定账户集。
...
*   **`prioritizationFee`** (`u64`): 在此槽位中至少有一个交易支付的最低优先费用（以每计算单元的微 Lamports 计），并符合 `lockedWritableAccounts` 过滤器（如果有）。值为 `0` 通常意味着在该槽位中没有交易（符合条件）支付了超出基础费用的额外优先费用，或者节点没有观察到给定账户的任何费用。
...
*   **费用单位：** 优先费用以每计算单元（CU）微Lamports（0.000001 Lamports）表示。
*   **缓存窗口：** RPC节点通常会缓存这些费用大约150个区块。这意味着您查看的是一个相对较短的历史窗口（大约1-2分钟）。
*   **零费用：** 一个`prioritizationFee`的`0`并不一定意味着没有支付费用，而是对于给定的槽位和账户，采样的交易不包括优先费用，或者低于节点认为重要的阈值。
*   **战略使用：** 不要只选择最近的最高费用。分析分布（例如，中位数，非零费用的第75百分位数）以做出具有成本效益的选择。如果区块已经充满高优先级交易，超额支付并不能保证更快的包含。
