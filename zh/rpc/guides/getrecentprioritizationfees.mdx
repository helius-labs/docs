...
[`getRecentPrioritizationFees`](https://www.helius.dev/docs/api-reference/rpc/http/getrecentprioritizationfees) RPC 方法提供了对 Solana 网络中最近区块中支付的优先费用的洞察。通过检查这些费用，开发者可以更好地决定在交易中附加多少额外费用（[优先费用](https://www.helius.dev/blog/priority-fees-understanding-solanas-transaction-fee-mechanics)），以增加其交易被及时处理的可能性，特别是在网络活动高峰期。
...
*   **零费用：** `prioritizationFee`的`0`并不一定意味着没有支付费用，而是对于给定的槽位和账户，采样的交易不包括优先费用，或者低于节点认为重要的阈值。
