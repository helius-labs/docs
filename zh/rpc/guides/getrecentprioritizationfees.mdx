...
[`getRecentPrioritizationFees`](https://www.helius.dev/docs/api-reference/rpc/http/getrecentprioritizationfees) RPC 方法提供了对 Solana 网络中最近区块中支付的优先费用的洞察。通过检查这些费用，开发者可以更好地决定在交易中附加多少额外费用（[优先费用](https://www.helius.dev/blog/priority-fees-understanding-solanas-transaction-fee-mechanics)），以提高交易被及时处理的可能性，特别是在网络活动高峰期。
...
1.  **`lockedWritableAccounts`** (`array` of `string`, 可选):
    *   一个 base-58 编码的账户公钥数组，您的交易打算对其进行写锁定。
    *   最多可以提供 128 个地址。
...
JSON-RPC 响应的 `result` 字段是一个优先费用对象的数组。每个对象详细说明了来自特定最近插槽的费用，并具有以下结构：

*   **`slot`** (`u64`): 处理贡献此费用数据的交易的插槽编号。
*   **`prioritizationFee`** (`u64`): 在此插槽中至少有一笔交易支付的最低优先费用（以每计算单元的微 Lamports 计），并符合 `lockedWritableAccounts` 过滤器（如果有）。值为 `0` 通常意味着在该插槽中没有交易（符合条件）支付了超出基础费用的额外优先费用，或者节点没有观察到给定账户的任何费用。
...
*   **费用单位：** 优先费用以每计算单元（CU）0.000001 Lamports（微Lamports）表示。
