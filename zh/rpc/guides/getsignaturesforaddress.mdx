...
* **`signature`** (`string`)：Base-58 编码的交易签名。 * **`slot`** (`u64`)：处理交易的槽位。 * **`err`** (`object` | `null`)：如果交易失败，则为错误对象；如果成功，则为 `null`。 * **`memo`** (`string` | `null`)：与交易相关的备注（如果有）。 * **`blockTime`** (`i64` | `null`)：包含交易的区块的估计生产时间，以 Unix 时间戳表示（自纪元以来的秒数）。如果不可用，则为 `null`。 * **`confirmationStatus`** (`string` | `null`)：交易的确认状态（例如，`processed`，`confirmed`，`finalized`）。如果不可用（例如，对于较旧的 Helius 响应），则为 `null`。
