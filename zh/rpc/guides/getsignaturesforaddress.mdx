 ...
 * **`before`** (`string`，可选)：一个base-58编码的交易签名。如果提供，查询将从此签名前的交易开始搜索。
...
* **`signature`** (`string`)：base-58 编码的交易签名。 * **`slot`** (`u64`)：交易被处理的槽位。 * **`err`** (`object` | `null`)：如果交易失败则为错误对象，成功则为 `null`。 * **`memo`** (`string` | `null`)：与交易相关的备忘录（如果有）。 * **`blockTime`** (`i64` | `null`)：包含交易的区块的估计生产时间，以 Unix 时间戳表示（自纪元以来的秒数）。如果不可用则为 `null`。 * **`confirmationStatus`** (`string` | `null`)：交易的确认状态（例如，`processed`，`confirmed`，`finalized`）。如果不可用（例如，对于较旧的 Helius 响应）则为 `null`。
