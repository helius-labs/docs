...
*   **Epoch 边界：** 领导者计划是为整个 epoch 确定的，详细信息请参阅我们的 [Slots, Blocks, and Epochs](https://www.helius.dev/blog/solana-slots-blocks-and-epochs) 指南。此方法允许您查询该计划的部分内容。如果您的范围跨越了 epoch 边界，返回的领导者仍将基于 `startSlot` 所在的 epoch 的计划，直到您提供的限制或该 epoch 已知计划的结束，如果 RPC 节点不为该调用进一步预测。
