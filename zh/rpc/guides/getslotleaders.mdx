...
*   **Epoch 边界：** 领导者计划是为整个 epoch 确定的，详细信息请参阅我们的 [Slots, Blocks, and Epochs](https://www.helius.dev/blog/solana-slots-blocks-and-epochs) 指南。此方法允许您查询该计划的部分内容。如果您的范围跨越了 epoch 边界，返回的领导者仍将基于 `startSlot` 所在的 epoch 的计划，直到您提供的限制或该 epoch 已知计划的结束，如果 RPC 节点没有进一步预测该调用。
...
*   **与 `getLeaderSchedule` 的区别：** 虽然 `getSlotLeaders` 为您提供了一个范围的直接列表，但 `getLeaderSchedule` 提供了整个 epoch 的完整计划，将验证者身份映射到该 epoch 内所有分配的槽。如果您只需要特定有限范围的顺序列表，`getSlotLeaders` 更为直接。
