...
*   **`foundationTerm`**：分配 `foundation` 的年限。
...
*   **静态参数：** 这些通胀参数在集群的创世时或通过网络升级设置，通常不期望在没有正式治理过程和网络更新的情况下频繁更改。
*   **对质押奖励的影响：** 这些参数共同决定了整体通胀计划，进而影响质押SOL的年百分比率（APR）。
*   **基金会分配：** `foundation` 和 `foundationTerm` 参数描述了在特定时期内用于支持生态系统发展和运营的通胀中分配给Solana基金会的特定部分。

本指南概述了如何使用 `getInflationGovernor` RPC 方法来检索和理解管理Solana网络通胀的核心参数。