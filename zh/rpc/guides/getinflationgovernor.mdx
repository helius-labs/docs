...
*   **`commitment`**（字符串，可选）：指定查询账本时使用的[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)。如果未提供，则使用节点的默认承诺（通常为 `finalized`）。
...
*   **基金会分配：** `foundation` 和 `foundationTerm` 参数描述了在特定时期内向 Solana 基金会定向的一部分通胀，以支持生态系统的发展和运营。
