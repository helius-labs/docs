...
*   **`taper`**：通胀率从 `initial` 降至 `terminal` 率的下降速度。这是通胀率本身的年度百分比下降（例如，0.15 的逐渐减少意味着通胀率每年下降 15%，直到达到终端利率）。
...
*   **基金会分配：** `foundation` 和 `foundationTerm` 参数描述了在特定时期内用于支持生态系统发展和运营的通胀部分分配给Solana基金会。

本指南概述了如何使用 `getInflationGovernor` RPC方法来检索和理解管理Solana网络通胀的核心参数。