...
* **理解纪元边界**：当通过`identity`查询而没有`range`时，返回的数据是当前纪元中节点处理的最新槽位的数据。如果您需要特定验证者的*完整*过去纪元的数据，您需要确定该纪元的`firstSlot`和`lastSlot`。
...
* **身份与所有验证者**：在没有`identity`的情况下查询可能会返回大量数据，特别是如果没有指定`range`或指定了一个宽的`range`时。请注意响应大小和处理要求。
