...
2.  **`filter`** (对象，必需)：一个 JSON 对象，**必须**指定 `mint` 或 `programId` 以过滤账户：
    ...
    *   **`programId`** (字符串)：拥有账户的代币程序的 base-58 编码公钥。通常是标准的 SPL Token Program (`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`) 或 Token-2022 Program (`TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb`)。

3.  **`options`** (对象，可选)：一个可选的配置对象，具有以下常见字段：
    *   **`commitment`** (字符串，可选)：指定[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)。
    *   **`encoding`** (字符串，可选)：账户数据的编码。强烈推荐使用 `"jsonParsed"`，因为它返回人类可读的账户信息。其他选项包括 `"base64"`, {/* INLINE_CODE_PLACEHOLDER_65db36f71bcb90b8df87389b69ce72f */}。如果未指定，默认为 `"base64"`。
...
*   **过滤要求：** 您*必须*在过滤参数中提供 `mint` 或 `programId`。如果没有这些过滤器之一，您无法查询所有代币类型的所有委托账户。
*   **编码：** 使用 `"jsonParsed"` 进行 `encoding` 选项的编码是高度推荐的，因为它将二进制账户数据解码为结构化的 JSON 格式，便于数据处理。
*   **性能：** 使用 `programId` 查询可能比使用 `mint` 查询更耗费资源，特别是当委托对许多不同的代币类型有权限时。一些 RPC 提供商可能对这种方法有更严格的速率限制。
