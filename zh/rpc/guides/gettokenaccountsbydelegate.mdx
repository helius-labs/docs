...
JSON-RPC 响应中的 `result.value` 字段是一个对象数组。每个对象代表一个以 `delegatePubkey` 作为委托并符合 `filter` 标准的代币账户。数组中的每个对象有两个字段：
...
*   **过滤器要求：** 在过滤器参数中，您*必须*提供 `mint` 或 `programId`。如果没有这些过滤器之一，您无法查询所有代币类型的所有委托账户。
