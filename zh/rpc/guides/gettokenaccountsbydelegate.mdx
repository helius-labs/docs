...
[`getTokenAccountsByDelegate`](https://www.helius.dev/docs/api-reference/rpc/http/gettokenaccountsbydelegate) RPC 方法检索所有已批准特定公钥作为代理的 SPL Token 账户。代理有权对代币账户执行某些操作，例如转移或销毁代币，直至授权的金额。
...
2.  **`filter`** (对象，必需)：一个 JSON 对象，**必须**指定 `mint` 或 `programId` 以过滤账户：
    ...
    *   **`programId`** (字符串)：拥有账户的代币程序的 base-58 编码公钥。通常是标准 SPL Token 程序 (`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`) 或 Token-2022 程序 (`TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb`)。
...
*   **过滤器要求：** 您*必须*在过滤器参数中提供 `mint` 或 `programId`。如果没有这些过滤器之一，您无法查询所有代币类型的所有委托账户。
