...
[`getClusterNodes`](https://www.helius.dev/docs/api-reference/rpc/http/getclusternodes) RPC 方法返回当前参与 Solana 集群的所有已知节点的列表。此信息可用于网络分析、发现替代 RPC 端点或从查询节点的角度了解当前网络拓扑。
...
* **`gossip`** (string | null): 节点的 gossip 服务的 IP 地址和端口。如果不可用，可以是 `null`。
* **`tpu`** (string | null): 节点的交易处理单元 (TPU) 的 IP 地址和端口。用于直接提交交易。如果不可用，可以是 `null`。
* **`rpc`** (string | null): 节点的 JSON-RPC 服务的 IP 地址和端口。如果此节点未启用或未公布 RPC 服务，可以是 `null`。
