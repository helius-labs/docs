...
*   `blockHeight` (数字 | null)：此区块的高度（从槽位 0 开始的链中在其之前的区块数），如果不可用则为 `null`。
...
**重要：**槽位号处理速度很快。下面使用的槽位号（`250000000`）是一个占位符。运行示例时，应将其替换为您知道存在于目标网络（例如，Devnet 或 Mainnet）上的最近确认槽位。您可以使用 Solana 区块浏览器找到最近的槽位号。
...
*   **槽位与区块高度：** 请记住，`getBlock` 接受的是 `slot` 数字作为输入，不一定是区块高度。虽然槽位是顺序的，但某些槽位可能会被领导者跳过。响应中的 `blockHeight` 字段表示在此之前的实际区块数量。
...
*   **区块不可用：** `null` 结果意味着在该槽位的区块未找到。这可能是因为槽位被跳过，区块尚未确认到您指定的 `commitment` 级别，或者 RPC 节点已从其分类账中修剪了该历史区块（对于较旧的槽位很常见）。
