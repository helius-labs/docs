...
2.  `config`（对象，可选）：包含以下字段的配置对象：
    ...
    *   `encoding`（字符串，可选）：交易数据的编码。如果 `transactionDetails` 是 `full` 或 `accounts`，则默认为 `json`，否则为 `base64`。
        ...
        *   `base64`
        *   `base64+zstd`
    *   `transactionDetails`（字符串，可选）：指定要返回的交易详细信息级别。默认为 `full`。
...
*   **槽位与区块高度：** 请记住，`getBlock` 接受一个 `slot` 数字作为输入，不一定是区块高度。虽然槽位是顺序的，但某些槽位可能会被领导者跳过。响应中的 `blockHeight` 字段指示在此之前的实际区块数量。
...
*   **选择 `transactionDetails`：** 
    ...
    *   `signatures` 如果您只需要列出区块中的交易，则很有用。
    *   `accounts` 可以是一个中间选择，如果您需要查看涉及哪些账户而不获取所有指令数据。
    *   `none` 很少见，但如果您只关心区块级元数据，如 `blockhash` 或 `rewards`，则可以使用。
*   **推荐使用 `jsonParsed` 进行编码：** 请求交易详情时，`jsonParsed` 提供最适合开发者的输出，并正确解析地址查找表中的账户，而 `json`（已弃用）则不。
