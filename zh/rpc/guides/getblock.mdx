 ...
 * `jsonParsed`：以解析的 JSON 格式返回交易和账户数据。建议使用此选项，因为它包括所有交易账户密钥（包括来自地址查找表的密钥）。
 * `base58`（慢）
 * `base64`
 * `base64+zstd`
...
**重要：**槽号处理速度很快。下面使用的槽号（`250000000`）是一个占位符。运行示例时，应将其替换为您知道在目标网络（例如，Devnet 或 Mainnet）上存在的最近确认槽。您可以使用 Solana 区块浏览器找到最近的槽号。
...
*   **`maxSupportedTransactionVersion` 是关键：** 要检查带有版本化交易的区块（现在是标准并使用地址查找表），您**必须**设置 `maxSupportedTransactionVersion: 0`（如果出现新标准，则设置更高版本）。忘记这一点会导致大多数现代区块出现错误。
...
*   **区块不可用：** `null` 结果表示在该槽位未找到区块。这可能是因为槽位被跳过，区块尚未确认到您指定的 `commitment` 级别，或者 RPC 节点已从其分类账中修剪了该历史区块（对于较旧的槽位很常见）。
*   **奖励信息：** 设置 `rewards: true` 是查看区块奖励分配给验证者（以及可能的质押者，取决于奖励类型）的必要条件。这会增加响应的大小。
*   **理解区块结构：** 要更深入地了解区块如何融入 Solana 的架构，请参阅 [理解 Solana 上的槽位、区块和纪元](https://www.helius.dev/blog/solana-slots-blocks-and-epochs)。