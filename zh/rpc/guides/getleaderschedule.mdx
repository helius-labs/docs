...
[`getLeaderSchedule`](https://www.helius.dev/docs/api-reference/rpc/http/getleaderschedule) RPC 方法返回特定纪元中区块生产领导权分配给验证者的情况。了解领导者计划对于网络分析、预测哪个验证者将在特定时间生产区块或与特定领导者交互的工具可能很有用。
...
*   `null`: 如果找不到请求的 `slot`（或如果未提供槽则为当前纪元）对应的纪元或其领导者计划不可用（例如，对于未来未计算的纪元）。
...
此示例获取给定验证器身份在包含插槽 `200000` 的纪元中的领导者计划。
...
*   **纪元边界：** 领导者计划在整个纪元内是固定的。您可以使用 `getEpochInfo` 找到纪元的开始和结束插槽。
*   **未来纪元：** 请求远未来纪元的计划可能会返回 `null`，如果网络尚未计算出来。
