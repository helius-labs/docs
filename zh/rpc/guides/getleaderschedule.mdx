...
2.  **`config`** (对象, 可选): 一个可以包含以下内容的配置对象：
    *   **`commitment`** (字符串, 可选): 指定[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)。如果未提供，将使用节点的默认承诺。
...
*   `null`: 如果未找到请求的 `slot`（或如果未提供槽则为当前纪元）对应的纪元或其领导者计划不可用（例如，对于未来未计算的纪元）。
...
*   **纪元边界：** 领导者计划在整个纪元中是固定的。您可以使用 `getEpochInfo` 来查找纪元的开始和结束插槽。
*   **未来纪元：** 请求远期纪元的计划可能会返回 `null`，如果网络尚未计算出它。
*   **相对插槽索引：** 请记住，响应中的插槽编号是相对于*请求*纪元的第一个插槽，而不是区块链上的绝对插槽编号。
