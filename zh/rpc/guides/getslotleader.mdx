...
1.  **`options`** (`object`, 可选): 一个可选的配置对象，包含以下字段：
    *   **`commitment`** (`string`, 可选): 指定查询的[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)。支持的值为 `finalized`, `confirmed`, 或 `processed`。如果省略，将使用 RPC 节点的默认承诺（通常为 `finalized`）。插槽领导者是基于与此承诺级别匹配的插槽确定的。
