...
由于程序可能拥有的账户数量可能很大，`getProgramAccounts` 提供了强大的过滤功能，帮助您缩小搜索范围，并高效地检索所需的数据。
...
2.  **`options`** (`object`, 可选): 一个配置对象，包含以下字段：
    ...
    *   **`encoding`** (`string`): 每个返回账户中`data`字段的编码。默认值为`"base64"`。
        *   `"base58"`: 二进制数据的较慢替代方案。
        *   `"base64"`: 二进制数据的标准base64编码。
        *   `"base64+zstd"`: base64编码的zstd压缩二进制数据。
        *   `"jsonParsed"`: 如果RPC节点有程序账户类型的解析器（例如，SPL Token, Stake），那么`data`字段将是一个结构化的JSON对象。这对于可读性和易用性非常推荐。
    *   **`filters`** (`array`): 应用于账户的过滤器对象数组。这对于性能和相关性至关重要。您可以使用最多4个过滤器。常见的过滤器包括：
        *   **`dataSize`** (`object`):
    ...
    *   **`dataSlice`** (`object`): 仅返回每个账户数据的特定切片。当您只需要部分数据时，这对于大账户非常有用。
...
*   **`account`** (`object`):
    ...
    *   `executable` (`boolean`): 账户是否可执行（即自身为程序）。
    *   `rentEpoch` (`u64`): 此账户下次需要支付租金的纪元。
    *   `space` (`u64`, 可选): 账户的数据长度，以字节为单位。有时称为 `data.length`，如果数据是缓冲区，或是解析结构的一部分。
...
此示例查找持有 USDC 的所有 SPL Token 账户。它使用 `dataSize` 过滤 token 账户（165 字节）并使用 `memcmp` 匹配偏移量 0 处的 USDC 铸币地址。
...
此示例查找由特定钱包地址拥有的所有 SPL Token 账户。它使用 `dataSize`（165 字节）和 `memcmp` 在偏移量 32（token 账户中存储所有者公钥的位置）。
...
- `memcmp`: 过滤在特定偏移量上匹配特定模式的账户
- `dataSize`: 按账户的精确数据大小过滤
- 多重过滤器：所有条件必须满足（逻辑与）

## 开发者提示

*   **性能:** `getProgramAccounts` 在 RPC 节点上可能会消耗大量资源，尤其是在没有过滤器或程序拥有许多账户的情况下。始终使用过滤器（`dataSize`, `memcmp`）和 `dataSlice` 来尽可能减少查询范围和响应大小。
*   **大型结果集:** 对于返回大量结果的查询，响应可能会被截断或超时。使用过滤来减少范围，或考虑使用 [`getProgramAccountsV2`](/zh/api-reference/rpc/http/getprogramaccountsv2) 以支持分页。
*   **速率限制:** 注意 RPC 提供者的速率限制，因为频繁或大量的 `getProgramAccounts` 调用可能会触及这些限制。
*   **数据布局知识:** 有效使用 `memcmp` 需要了解您正在查询的账户数据的字节布局。
*   **`jsonParsed` 可用性:** `jsonParsed` 编码依赖于 RPC 节点是否有特定程序账户类型的解析器。对于常见程序如 SPL Token 广泛支持。

`getProgramAccounts` 是开发人员查询和与程序拥有的账户集交互时不可或缺的方法。掌握其过滤选项是构建高效且稳健的 Solana 应用程序的关键。
...
- **更好的性能**: 防止超时并减少内存使用
- **分页行为**: 只有在没有返回账户时才表示分页结束。由于过滤，返回的账户可能少于限制 - 继续分页直到 `paginationKey` 为 null
