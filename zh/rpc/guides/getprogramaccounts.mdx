...
对于需要查询非常大规模程序账户集的应用程序，考虑使用 [`getProgramAccountsV2`](/zh/api-reference/rpc/http/getprogramaccountsv2)，它提供基于游标的分页支持，每个请求可配置页面大小最多为 10,000 个账户。
...
*   **`jsonParsed` 可用性：** `jsonParsed` 编码依赖于 RPC 节点是否具有特定程序账户类型的解析器。对于常见程序如 SPL Token 广泛支持。
