...
由于程序可能拥有的大量账户，`getProgramAccounts` 提供了强大的过滤功能，帮助您缩小搜索范围并高效地检索所需数据。

对于需要查询非常大规模程序账户的应用程序，考虑使用 [`getProgramAccountsV2`](/zh/api-reference/rpc/http/getprogramaccountsv2)，它提供基于游标的分页支持，每个请求可配置页面大小最多为 10,000 个账户。
...
2.  **`options`** (`object`, 可选): 一个配置对象，包含以下字段：
    ...
    *   **`filters`** (`array`): 一个应用于账户的过滤器对象数组。这对于性能和相关性至关重要。您可以使用最多4个过滤器。常见的过滤器包括：
        *   **`dataSize`** (`object`):
...
此示例查找持有USDC的所有SPL Token账户。它使用`dataSize`来过滤token账户（165字节），并使用`memcmp`在偏移量0匹配USDC铸币地址。
...
此示例查找由特定钱包地址拥有的所有SPL Token账户。它使用`dataSize`（165字节）和`memcmp`在偏移量32（token账户中存储所有者公钥的位置）。
...
*   **性能:** `getProgramAccounts` 在 RPC 节点上可能资源密集，尤其是在没有过滤器或程序有许多账户时。始终使用过滤器（`dataSize`, `memcmp`）和 `dataSlice` 来减少查询范围和响应大小。
*   **大型结果集:** 对于返回许多结果的查询，响应可能会被截断或超时。使用过滤来减少范围，或考虑使用 [`getProgramAccountsV2`](/zh/api-reference/rpc/http/getprogramaccountsv2) 以支持分页。
...
`getProgramAccounts` 是开发人员查询和与程序拥有的账户集交互的不可或缺的方法。掌握其过滤选项是构建高效和稳健的 Solana 应用程序的关键。
