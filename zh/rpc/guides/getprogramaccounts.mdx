...
对于需要查询非常大规模程序账户的应用程序，考虑使用 [`getProgramAccountsV2`](/zh/api-reference/rpc/http/getprogramaccountsv2)，它提供基于游标的分页支持，配置的页面大小每次请求最多可达 10,000 个账户。
...
*   **大结果集：** 对于返回许多结果的查询，响应可能会被截断或超时。使用过滤器减少范围，或考虑使用 [`getProgramAccountsV2`](/zh/api-reference/rpc/http/getprogramaccountsv2) 以支持分页。
