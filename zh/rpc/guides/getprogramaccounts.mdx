...
对于需要查询非常大程序账户集的应用程序，考虑使用 [`getProgramAccountsV2`](/zh/api-reference/rpc/http/getprogramaccountsv2)，它提供基于游标的分页支持，每个请求的页面大小可配置至最多 10,000 个账户。
...
2.  **`options`** (`object`, 可选): 一个配置对象，包含以下字段：
    ...
    *   **`filters`** (`array`): 应用于账户的过滤对象数组。这对于性能和相关性至关重要。您可以使用最多4个过滤器。常见的过滤器包括：
        *   **`dataSize`** (`object`):
...
*   **`account`** (`object`):
    ...
    *   `space` (`u64`, 可选): 账户的数据长度，以字节为单位。有时如果数据是缓冲区，或是解析结构的一部分，称为`data.length`。
...
此示例查找由特定钱包地址拥有的所有SPL Token账户。它使用`dataSize`（165字节）和`memcmp`在偏移量32处（Token账户中存储所有者公钥的位置）。
