...
JSON-RPC 响应的 `result` 字段是一个性能样本对象数组，以逆时间顺序返回（最新的样本在前）。每个对象具有以下结构：

*   **`slot`** (`u64`): 记录此性能样本的槽号。
*   **`numTransactions`** (`u64`): 在此 `slot` 之前的 `samplePeriodSecs` 中处理的交易总数（包括投票和非投票交易）。
*   **`numSlots`** (`u64`): 在此 `slot` 之前的 `samplePeriodSecs` 中处理的槽数。
*   **`samplePeriodSecs`** (`u16`): 采集此样本的持续时间（以秒为单位）（通常为 `60`）。
*   **`numNonVoteTransactions`** (`u64`): 在此 `samplePeriodSecs` 中处理的非共识投票交易的数量。
...
*   **节点变异性：**确切的数据可能会因不同RPC节点的同步状态和在采样时对网络的本地视图而略有不同。
