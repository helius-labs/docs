 ...
 * **`searchTransactionHistory`** (`boolean`, 可选): 如果是 `true`，RPC节点将搜索其完整的交易历史以查找签名。如果是 `false`（默认），它只搜索最近的状态缓存。对于旧的或可能被丢弃的交易，将此设置为 `true`。
 ...
 * 如果找到签名，则为包含以下字段的**对象**：
 * **`slot`** (`u64`): 交易被处理的槽位。
 * **`confirmations`** (`number` | `null`): 自交易被处理以来已确认的区块数。如果交易已完成，则为`null`（因为最终性意味着它不会被回滚，所以具体的确认计数不那么重要）。
 ...
 * **`null`**: 如果在状态缓存中找不到签名且`searchTransactionHistory`为`false`（或即使在历史搜索中也确实不存在）。
...
* **`null` 状态：** 给定签名的 `value` 数组中的 `null` 表示未找到其状态。这可能是因为它不在最近缓存中（如果 `searchTransactionHistory` 为 false），交易从未发生，或者即使使用 `searchTransactionHistory: true` 也太旧而不在节点的历史中。
