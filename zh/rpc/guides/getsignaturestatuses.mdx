...
*   **`null` 状态：** 给定签名的 `value` 数组中的 `null` 表示未找到其状态。这可能是因为它不在最近的缓存中（如果 `searchTransactionHistory` 为 false），交易从未落地，或者即使使用 `searchTransactionHistory: true` 也太旧而无法在节点历史中找到。
*   **`confirmations: null`**：这通常意味着交易已达到 `finalized` 状态。此时，特定确认次数的概念不太相关，因为该区块被认为是不可逆的。
