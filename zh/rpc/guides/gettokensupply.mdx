...
[`getTokenSupply`](https://www.helius.dev/docs/api-reference/rpc/http/gettokensupply) RPC 方法返回特定 SPL 代币铸造的总供应量。这对于了解已创建的代币总量至关重要。
...
*   **`uiAmountString`** (字符串)：调整为代币的 `decimals` 后的代币总供应量，以字符串形式表示。这是代币总供应量的最用户友好表示。
...
*   **供应量通常不可变：** 对于大多数 SPL 代币，一旦铸造，从铸造账户本身的角度来看，总供应量是固定的，除非铸造有特定的铸造权限可以创建更多代币（或销毁它们，尽管销毁通常发生在代币账户，而不是铸造的供应量直接）。
*   **`decimals` 是关键：** 始终使用 `decimals` 字段来正确解释 `amount` 或 `uiAmountString`。
*   **数据来源：** 此方法直接查询铸造账户以获取其供应信息。

本指南提供了使用`getTokenSupply` RPC方法有效查询Solana上SPL代币供应所需的信息。