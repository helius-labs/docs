...
对于拥有大量代币组合的钱包，考虑使用 [`getTokenAccountsByOwnerV2`](/api-reference/rpc/http/gettokenaccountsbyownerv2)，它提供基于游标的分页支持，每个请求最多可配置 10,000 个账户。
...
*   **性能：** 如果一个所有者拥有非常多的代币账户（特别是仅通过 `programId` 过滤时），响应可能会很大。在这种情况下，使用 [`getTokenAccountsByOwnerV2`](/api-reference/rpc/http/gettokenaccountsbyownerv2)，它提供内置的分页支持。
...
对于持有大量代币的钱包，使用 [`getTokenAccountsByOwnerV2`](/api-reference/rpc/http/gettokenaccountsbyownerv2)，它提供：
