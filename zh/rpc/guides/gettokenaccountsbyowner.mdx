...
*   **过滤要求：** 你*必须*在过滤器中提供一个`mint`或一个`programId`。没有这些主要过滤器之一，无法查询所有代币类型的所有者的所有代币账户。
...
- **基于游标的分页**：设置`limit`（1-10,000）并使用`paginationKey`浏览结果
- **增量更新**：使用`changedSinceSlot`仅获取自特定槽位以来修改的代币账户
- **更好的性能**：防止超时并实现实时投资组合跟踪
- **分页行为**：分页结束仅在没有返回代币账户时指示。由于过滤，返回的账户可能少于限制 - 继续分页直到`paginationKey`为null
