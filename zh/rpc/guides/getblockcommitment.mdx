 ...
 * 数组的索引 `i` 显示对区块投票的权益，考虑到对区块本身及其后代最多 `i` 层的投票。
 * 如果找不到区块或其承诺信息不可用（例如，它太旧并从承诺跟踪中删除），此字段将为 `null`。
* `totalStake`（数字）：
 * 在处理此区块的槽位上的集群总活跃权益（以 lamports 为单位）。此值用于计算已承诺给区块的权益百分比。
...
*   **解释 `commitment` 数组：**
    ...
    *   如果 `commitment[i] / totalStake >= 2/3`（超级多数），您可以在深度 `i` 处评估最终性。
...
*   **修剪：** 请注意，RPC 节点可能会修剪旧的承诺信息，导致旧槽位的 `null` 结果。

## 示例：获取区块承诺信息

让我们尝试在 Devnet 上获取一个示例槽位号的承诺信息。
**重要：** 槽位号处理速度很快。下面使用的槽位号（`250000000`）是一个占位符。当您运行示例时，应将其替换为您知道存在于目标网络（例如，Devnet 或 Mainnet）上的最近确认槽位。您可以使用 Solana 区块浏览器找到最近的槽位号。
