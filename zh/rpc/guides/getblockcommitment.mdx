...
[`getBlockCommitment`](https://www.helius.dev/docs/api-reference/rpc/http/getblockcommitment) RPC 方法提供有关 Solana 分类账中特定区块的[承诺](https://www.helius.dev/blog/solana-commitment-levels)状态的信息。这对于了解一个区块的最终确定程度很有用，基于对其投票的权益。
...
* `totalStake`（数字）：
 * 在处理此区块的槽位中集群的总活跃权益（以 lamports 为单位）。此值用于计算已承诺给区块的权益百分比。
