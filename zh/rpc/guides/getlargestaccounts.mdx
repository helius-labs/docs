...
* **`commitment`** (字符串，可选)：指定查询分类帐时使用的[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)。如果未提供，则使用节点的默认承诺。
* **`filter`** (字符串，可选)：按账户类型过滤结果。接受的值有：
 * `circulating`：返回属于流通供应部分的最大账户。
 * `nonCirculating`：返回不属于流通供应部分的最大账户（例如，锁定账户、基金会账户）。
 如果省略，则所有账户都被视为不带此特定过滤器。
