...
*   **`context`** (对象)：
    *   **`slot`** (u64)：评估费用的槽位。
*   **`value`** (u64 | null)：以 lamports 为单位的估算费用。如果无法确定费用（例如，消息中使用的区块哈希过旧或无效），则可能为 `null`。
...
*   **最近的区块哈希：** 消息必须使用最近的区块哈希构建。如果区块哈希过旧，响应中的 `value` 可能为 `null`。
*   **费用与优先费用：** 此方法返回基础网络费用。不包括您可能添加到交易中的任何额外优先费用，以在网络拥堵时增加其快速处理的可能性。使用 `getRecentPrioritizationFees` 估算[优先费用](https://www.helius.dev/blog/priority-fees-understanding-solanas-transaction-fee-mechanics)。
*   **Lamports：** 费用以 lamports 返回（1 SOL = 1,000,000,000 lamports）。
*   **空值：** 费用的 `null` 值可能表示消息存在问题（例如，无效的区块哈希，格式错误的消息）或节点无法在给定的承诺级别或槽位上计算费用。
