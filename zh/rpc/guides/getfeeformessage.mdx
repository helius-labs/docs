...
*   **`context`** (对象)：
    *   **`slot`** (u64)：评估费用的槽位。
*   **`value`** (u64 | null)：以 lamports 为单位的估算费用。如果无法确定费用（例如，消息中使用的 blockhash 过旧或无效），则可能为 `null`。
...
*   **最近的 Blockhash：** 消息必须使用最近的 blockhash 构建。如果 blockhash 过旧，响应中的 `value` 可能为 `null`。
