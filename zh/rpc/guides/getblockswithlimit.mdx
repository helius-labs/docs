...
1.  **`start_slot`** (u64, 必需): 要考虑的第一个槽（包括）。
2.  **`limit`** (u64, 必需): 要返回的最大区块槽数量。查询的槽总数（从 `start_slot` 到其后的 `limit` 区块）**不得超过 500,000 个槽**。
3.  **`commitment`** (字符串, 可选): 指定查询的承诺级别。如果省略，则使用节点的默认承诺。这作为配置对象中的唯一字段传递，作为最后一个参数。
...
本指南提供了如何使用 `getBlocksWithLimit` RPC 方法在 Solana 网络上列出特定数量的已确认区块槽的清晰概述。