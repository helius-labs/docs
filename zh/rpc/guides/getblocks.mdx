...
此示例获取插槽 `250000000` 和 `250000010` 之间的已确认区块插槽列表。
...
此示例从 `260000000` 开始获取已确认的区块插槽，直到节点确认的最新区块（遵循从起始插槽开始的 500,000 插槽范围限制）。
...
本指南清晰概述了如何使用`getBlocks` RPC 方法列出 Solana 网络上已确认的区块槽位。