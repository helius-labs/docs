...
[`getBlocks`](https://www.helius.dev/docs/api-reference/rpc/http/getblocks) RPC 方法允许您检索指定起始槽和可选结束[槽](https://www.helius.dev/blog/solana-slots-blocks-and-epochs)之间的已确认区块槽编号列表。当您需要知道在特定范围内哪些区块已被确认而不获取每个区块的完整内容时，这很有用。
...
JSON-RPC 响应的 `result` 字段将是一个 u64 整数数组。数组中的每个整数表示指定范围内的已确认区块槽编号。
...
*   **范围限制：** 记住 500,000 插槽的范围限制。请求更大范围将导致错误。
...
本指南清晰概述了如何使用 `getBlocks` RPC 方法列出 Solana 网络上的已确认区块插槽。