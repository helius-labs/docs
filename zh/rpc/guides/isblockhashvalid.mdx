...
[`isBlockhashValid`](https://www.helius.dev/docs/api-reference/rpc/http/isblockhashvalid) RPC 方法检查先前获得的区块哈希是否仍被网络视为有效。区块哈希有一个有限的生命周期（大约 2 分钟或 150 个区块），之后引用它们的交易将被拒绝。

此方法对于在提交交易前保留 blockhash 一段时间的应用程序至关重要，以确保交易不会因 blockhash 过期而失败。

**版本说明：** 此方法在 `solana-core` v1.9 及更新版本中可用。对于运行 `solana-core` v1.8 或更早版本的节点，您应该使用 `getFeeCalculatorForBlockhash`，它除了提供费用信息外，还隐含地指示区块哈希的有效性（如果区块哈希过旧，将会出错）。
...
1.  **`blockhash`** (字符串，必需)：要检查的区块哈希，作为 base-58 编码的字符串。
