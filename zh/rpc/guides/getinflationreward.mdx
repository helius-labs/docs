...
2.  **`config`**（对象，可选）：包含以下可选字段的配置对象：
    *   **`commitment`**（字符串，可选）：指定[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)。如果未提供，默认为 `finalized`。
    *   **`epoch`**（整数，可选）：要获取奖励的纪元编号。如果省略，RPC 节点通常会使用最近完成的已分发奖励的纪元。
    *   **`minContextSlot`**（整数，可选）：请求可以评估的最小槽位。这确保查询针对至少处理到此槽位的分类账状态进行。
...
*   **空结果:** 地址的 `null` 结果表示在指定的纪元中未找到该地址的奖励。这可能是因为账户不符合条件（例如，未达到足够的质押账户），奖励为零，或该账户在当时不存在。
