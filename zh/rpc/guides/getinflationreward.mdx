 ...
 * **`minContextSlot`**（整数，可选）：请求可以评估的最小槽位。这确保查询是在至少处理到该槽位的账本状态下进行的。
...
* **奖励的时间：** 通胀奖励在纪元结束时计算，并在下一个纪元开始时应用。`effectiveSlot` 指示何时发生。
* **空结果：** 地址的 `null` 结果表示在指定的纪元中没有找到该地址的奖励。这可能是因为账户不符合条件（例如，不是足够抵押的抵押账户）、奖励为零或账户当时不存在。
