...
[`getEpochSchedule`](https://www.helius.dev/docs/api-reference/rpc/http/getepochschedule) RPC 方法返回来自集群创世配置的[epoch](https://www.helius.dev/blog/solana-slots-blocks-and-epochs)计划信息。此数据定义了 epoch 的结构，包括其长度以及如何相对于 epoch 的开始确定领导者计划。
...
*   **预热期：** 如果 `warmup` 是 `true`，那么在 `firstNormalEpoch` 之前的 epochs 将比 `slotsPerEpoch` 拥有更少的 slots。预热 epoch 长度的精确计算是 `2^N * MINIMUM_SLOTS_PER_EPOCH`，其中 N 是 epoch 编号（从 0 开始）直到达到 `firstNormalEpoch`。通常 `MINIMUM_SLOTS_PER_EPOCH` 是 32。
