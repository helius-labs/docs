...
[`getEpochSchedule`](https://www.helius.dev/docs/api-reference/rpc/http/getepochschedule) RPC 方法返回来自集群创世配置的[epoch](https://www.helius.dev/blog/solana-slots-blocks-and-epochs)计划信息。此数据定义了 epoch 的结构，包括其长度以及如何根据 epoch 的开始来确定领导者计划。

理解 epoch 计划对于需要与网络事件对齐、预测 epoch 边界或理解领导者轮换机制的应用程序至关重要。

## 常见用例

* **预测 Epoch 边界：** 确定一个 epoch 中的插槽数量，以估计当前 epoch 何时结束以及下一个 epoch 何时开始。
* **领导者计划计算：** 理解 `leaderScheduleSlotOffset` 以了解为即将到来的 epoch 提前多长时间生成领导者计划。
* **分析网络初始化：** 观察 `warmup`, `firstNormalEpoch` 和 `firstNormalSlot` 以了解集群 epoch 长度的初始启动阶段（如果适用）。
...
*   **预热期：** 如果 `warmup` 是 `true`，那么在 `firstNormalEpoch` 之前的纪元将比 `slotsPerEpoch` 拥有更少的插槽。预热纪元长度的精确计算为 `2^N * MINIMUM_SLOTS_PER_EPOCH`，其中 N 是纪元编号（从 0 开始），直到达到 `firstNormalEpoch`。`MINIMUM_SLOTS_PER_EPOCH` 通常为 32。

本指南提供了使用 `getEpochSchedule` RPC 方法所需的信息，以了解 Solana 集群中纪元的基本时间和结构。