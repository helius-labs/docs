...
1.  **`options`** (object, optional): 一个可选的配置对象，可以包括：
    *   **`commitment`** (string, optional): 指定查询的[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)（例如，`"finalized"`, `"confirmed"`, `"processed"`）。如果未提供，则使用节点的默认承诺。
...
*   **递增值：** 交易计数是一个单调递增的值。
*   **承诺级别：** 返回的计数取决于所选择的 `commitment` 级别。一个 `processed` 承诺可能会产生更高、更接近实时的计数，而 `finalized` 提供了防止回滚的保证。
