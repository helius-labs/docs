...
* **通货膨胀监测：** 尽管 `getInflationRate` 和 `getInflationGovernor` 提供了更直接的通胀数据，`getSupply` 可以提供更广泛的背景。
 ...
 * **`nonCirculatingAccounts`** (字符串数组，可选)：持有非流通 SOL 的账户公钥数组（以 base58 编码的字符串）。如果请求中 `excludeNonCirculatingAccountsList` 设置为 `true`，则省略此字段。
* **`context`**：包含以下内容的对象：
 * **`slot`** (u64)：检索信息时的槽位。
