...
*   **`context`** (object): 一个包含以下字段的 `RpcResponseContext` 对象：
    *   **`slot`** (`u64`): 数据检索时的槽位。
*   **`value`** (`u64`): 最小的 stake 委托数量，以 lamports 为单位。
...
*   **Lamports vs. SOL:** 返回的值以 lamports 为单位。请记住，1 SOL = 1,000,000,000 lamports。
...
本指南应帮助您有效地使用 `getStakeMinimumDelegation` RPC 方法来检索 Solana 上所需的最小 stake 委托。