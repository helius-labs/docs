...
[`getStakeMinimumDelegation`](https://www.helius.dev/docs/api-reference/rpc/http/getstakeminimumdelegation) RPC 方法返回在 Solana 网络上创建新的[质押](https://www.helius.dev/blog/how-to-stake-solana)委托所需的当前最小 lamports 数量。由于网络治理或更新，此值可能会随时间变化。
...
*   **`context`**（对象）：一个 `RpcResponseContext` 对象，包含以下字段：
    *   **`slot`**（`u64`）：检索数据时的槽位。
*   **`value`**（`u64`）：最小质押委托金额，以 lamports 为单位。
...
*   **动态值：** 最小委托金额可能会因网络参数而变化。建议定期查询此值或在关键操作前查询，而不是硬编码它。了解更多关于[质押](https://www.helius.dev/blog/how-to-stake-solana)的详细指南。
