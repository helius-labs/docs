...
*   **`value`** (`array`):
    ...
    *   每个元素将是：
        *   `null`: 如果指定公钥的账户不存在或该特定账户发生错误。
        *   一个具有以下字段的 **账户对象**：
            *   `lamports` (`u64`): 账户拥有的 lamports 数量。
            *   `owner` (`string`): 拥有该账户的程序的 base-58 编码公钥。
            *   `data` (`array` 或 `object`): 账户数据。如果 `encoding` 是 `jsonParsed` 并且存在解析器，这将是一个 JSON 对象。否则，它通常是一个数组 `["encoded_string", "encoding_format"]` (例如，`["SGVsbG8=", "base64"]`)。
...
*   **原子性：** 请求不是原子的，这意味着如果一个账户查询失败，其他的可能仍然成功。检查 `value` 数组中的每个元素以获取 `null`。
*   **`jsonParsed` 便利性：** 在处理常见账户类型（如SPL Token账户）时，强烈推荐使用 `jsonParsed` 编码，因为它可以避免手动反序列化。
