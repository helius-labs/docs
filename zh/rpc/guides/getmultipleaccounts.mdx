...
[`getMultipleAccounts`](https://www.helius.dev/docs/api-reference/rpc/http/getmultipleaccounts) RPC 方法是一种高效的方式，可以同时获取一组 Solana 账户的信息。与其为每个账户单独发起 `getAccountInfo` 请求，`getMultipleAccounts` 允许您批量处理这些请求，从而减少网络开销并提高应用程序的响应速度。
...
通过利用`getMultipleAccounts`，您可以构建更高性能和可扩展的Solana应用程序。