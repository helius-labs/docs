...
2.  **`commitment`** (`object`, 可选): 指定查询的[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)。如果省略，则使用 RPC 节点的默认承诺。
    *   字段:
...
*   **租金随数据大小变化:** 账户数据越大，免租最低限额就越高。
*   **网络参数:** 租金的基础成本（每字节年 lamports）是一个网络参数，理论上可以通过集群投票改变，但这很少见。`getMinimumBalanceForRentExemption` 方法将始终根据集群的状态返回当前值。
*   **Lamports 与 SOL:** 响应以 lamports 为单位。请记住，1 SOL = 1,000,000,000 lamports。
*   **长久性的重要性:** 确保账户免租对于 Solana 区块链上数据的长期持久性至关重要。
