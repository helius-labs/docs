...
[`getInflationRate`](https://www.helius.dev/docs/api-reference/rpc/http/getinflationrate) RPC 方法返回当前纪元的[通胀](https://www.helius.dev/blog/solana-issuance-inflation-schedule)率的详细信息。这包括总通胀率、分配给验证者的部分、分配给基金会的部分以及这些利率适用的纪元编号。
...
*   **纪元特定:** 返回的值特定于查询时的当前纪元。这些利率是从 `getInflationGovernor` 定义的整体通胀计划中得出的，但代表了这个特定纪元的年化利率。
*   **年化利率:** 这些利率通常以年化百分比表示，即使它们适用于当前纪元。
*   **动态值:** 虽然基础通胀计划 (`getInflationGovernor`) 很少变化，但计算出的 `getInflationRate` 将随着整体通胀的逐渐减少而在每个纪元中变化。
