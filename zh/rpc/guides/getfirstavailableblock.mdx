...
[`getFirstAvailableBlock`](https://www.helius.dev/docs/api-reference/rpc/http/getfirstavailableblock) RPC 方法返回尚未被查询节点从账本中清除的最低（最旧）确认区块的槽号。这对于了解特定节点当前存储的历史数据范围非常有用。
...
本指南解释了如何使用`getFirstAvailableBlock` RPC 方法来识别 Solana 节点上最早可用的区块槽。