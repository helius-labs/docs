...
[`getEpochInfo`](https://www.helius.dev/docs/api-reference/rpc/http/getepochinfo) RPC 方法返回当前纪元的信息。这对于了解网络的当前状态、网络在当前纪元中的进展程度以及下一个纪元可能何时开始非常有用。要更好地理解 Solana 上的[纪元、插槽和区块](https://www.helius.dev/blog/solana-slots-blocks-and-epochs)如何协同工作，请参阅我们的详细文章。
...
*   **`commitment`** (字符串，可选)：指定查询分类账时使用的[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)。
    *   `finalized`：节点将查询被集群的绝大多数确认达到最大锁定的最新区块。
    *   `confirmed`：节点将查询集群的绝大多数投票的最新区块。
    *   `processed`：节点将查询其最新的区块。请注意，该区块可能不完整。
    *   如果未提供，默认承诺为 `finalized`。
*   **`minContextSlot`** (数字，可选)：请求可以评估的最小插槽。这可以用于确保响应来自足够新的状态。

## 响应结构

JSON-RPC 响应的 `result` 字段将是一个包含以下字段的对象：

*   **`absoluteSlot`** (u64)：当前的绝对插槽号。
*   **`blockHeight`** (u64)：当前区块高度。
*   **`epoch`** (u64)：当前纪元号。
*   **`slotIndex`** (u64)：相对于当前纪元开始的当前插槽。
*   **`slotsInEpoch`** (u64)：当前纪元中的插槽总数。
...
*   **`transactionCount` 可用性：** 如果节点没有此信息或未跟踪特定承诺级别，`transactionCount` 字段可能为 `null`。
*   **纪元长度：** `slotsInEpoch` 的数量可能会有所不同。您可以使用 `getEpochSchedule` 获取有关纪元计划的更多详细信息。

本指南提供了使用 `getEpochInfo` RPC 方法检索 Solana 网络上当前纪元详细信息所需的信息。