...
[`getEpochInfo`](https://www.helius.dev/docs/api-reference/rpc/http/getepochinfo) RPC 方法返回有关当前 epoch 的信息。这对于了解网络的当前状态、网络在当前 epoch 中的进展程度以及下一个 epoch 可能何时开始非常有用。要更好地理解 [epochs、slots 和 blocks](https://www.helius.dev/blog/solana-slots-blocks-and-epochs) 在 Solana 上如何协同工作，请参阅我们的详细文章。
...
*   **承诺级别：** 选择 `commitment` 会影响数据的及时性和最终性。`processed` 是最快但最不安全的，而 `finalized` 是最安全但可能稍有延迟。了解更多关于 [承诺级别](https://www.helius.dev/blog/solana-commitment-levels) 的详细信息。
