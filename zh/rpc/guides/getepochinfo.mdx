...
[`getEpochInfo`](https://www.helius.dev/docs/api-reference/rpc/http/getepochinfo) RPC 方法返回当前 epoch 的信息。这对于了解网络的当前状态、网络在当前 epoch 中的进展程度以及下一个 epoch 可能何时开始非常有用。要更好地理解 Solana 上 [epochs, slots, 和 blocks](https://www.helius.dev/blog/solana-slots-blocks-and-epochs) 如何协同工作，请参阅我们的详细文章。
...
*   **`commitment`** (string, optional): 指定查询账本时使用的[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)。
...
*   **`minContextSlot`** (number, optional): 请求可以评估的最小 slot。这可以用于确保响应来自足够新的状态。
...
*   **`slotsInEpoch`** (u64): 当前 epoch 中的总 slot 数量。
