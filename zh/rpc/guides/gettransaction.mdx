...
*   **`meta`** (object | null): 包含交易执行元数据的对象。如果交易在处理前失败或元数据不可用，可以是`null`。
    ...
    *   **`preBalances`** (array of u64): 交易处理*之前*涉及账户的 lamport 余额。
    *   **`postBalances`** (array of u64): 交易处理*之后*涉及账户的 lamport 余额。
    ...
    *   **`loadedAddresses`** (object, optional): 指定从地址查找表加载的账户用于此交易。包含`writable`和`readonly`的公钥数组。
