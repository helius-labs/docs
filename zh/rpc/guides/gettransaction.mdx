...
如果未找到交易（例如，尚未处理或签名不正确）或未确认到指定的承诺级别，该方法返回 `null`。否则，它返回一个包含以下字段的对象：
...
*   **`meta`** (object | null)：包含交易执行元数据的对象。如果交易在处理前失败或元数据不可用，可以是 `null`。
    ...
    *   **`preBalances`** (array of u64)：交易处理*之前*涉及账户的Lamport余额。
    *   **`postBalances`** (array of u64)：交易处理*之后*涉及账户的Lamport余额。
    ...
    *   **`loadedAddresses`** (object, optional)：指定为此交易从地址查找表加载的账户。包含 `writable` 和 `readonly` 公钥数组。
    *   **`returnData`** (object, optional)：通过 `sol_set_return_data` 和 `sol_get_return_data` 返回的交易数据。包含 `programId` (string) 和 `data` (array: `[string, encoding]`)。
...
*   **`jsonParsed` vs. `json`:** 虽然 `jsonParsed` 非常方便，但解析支持取决于 RPC 节点对特定程序的能力。如果程序未被识别，即使使用 `jsonParsed`，其指令可能会回退到较少解析的格式。
