...
[`getTransaction`](https://www.helius.dev/docs/api-reference/rpc/http/gettransaction) RPC 方法允许您通过提供其签名来检索已确认交易的详细信息。这包括交易的插槽、区块时间、元数据（如费用、状态和余额变化）以及交易结构本身。
 ...
 * 如果 Helius 未指定，默认为 `"json"`，但 Solana 默认可能不同。最好指定此项。
...
*   **`meta`** (object | null)：包含有关交易执行的元数据的对象。如果交易在处理前失败或元数据不可用，可以是`null`。
    ...
    *   **`innerInstructions`** (array of objects | null)：在此交易中作为CPI（跨程序调用）的一部分执行的指令数组。
    ...
    *   **`returnData`** (object, optional)：通过交易返回的数据，包含`sol_set_return_data`和`sol_get_return_data`。包含`programId`（字符串）和`data`（数组：`[string, encoding]`）。
