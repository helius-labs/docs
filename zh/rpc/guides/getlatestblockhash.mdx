...
[`getLatestBlockhash`](https://www.helius.dev/docs/api-reference/rpc/http/getlatestblockhash) RPC 方法对于在 Solana 网络上准备和发送交易至关重要。它检索节点处理的最新 blockhash，以及该 blockhash 将保持有效的最后区块高度。有关如何有效使用 blockhashes 并确保您的[交易成功](https://www.helius.dev/blog/how-to-land-transactions-on-solana)的更多详细信息，请参阅我们的综合指南。
...
*   **交易构建：** 在签署和发送新交易之前，获取最近的 blockhash 以包含在交易中。
*   **交易生命周期管理：** 使用 `lastValidBlockHeight` 了解引用获取的 blockhash 的交易预计能保持有效的时间。
*   **预检：** 虽然 `simulateTransaction` 可以隐式执行此操作，但应用程序可能会获取 blockhash 以手动准备交易进行模拟。

## 请求参数

此方法可以选择性地接受一个包含以下参数的配置对象：

*   **`commitment`** (字符串，可选)：指定查询的[承诺级别](https://www.helius.dev/blog/solana-commitment-levels)。如果省略，则使用节点的默认承诺。对于交易，通常使用 `confirmed` 或 `finalized` 以确保 blockhash 来自链中相对稳定的部分。
*   **`minContextSlot`** (整数，可选)：请求可以评估的最小槽位。这确保查询针对已处理至少到此槽位的分类帐状态进行。
...
* **`lastValidBlockHeight`** (u64)：区块高度，在此高度下，`blockhash` 将过期。引用此 `blockhash` 的交易在网络达到此 `lastValidBlockHeight` 之前有效。
...
* **区块哈希有效性：** 区块哈希在有限时间内有效，大约 2 分钟（尽管这可能会有所不同）。交易必须在其 `lastValidBlockHeight` 通过之前由网络确认。如果自上次获取以来已经过了相当长的时间，请始终获取新的区块哈希。
