...
*   **`minContextSlot`** (整数，可选)：请求可以评估的最小槽位。这确保查询针对已处理到至少此槽位的账本状态进行。
...
* **`lastValidBlockHeight`** (u64)：区块高度，在此高度上，`blockhash` 将过期。引用此 `blockhash` 的交易在网络达到此 `lastValidBlockHeight` 之前有效。
