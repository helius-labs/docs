...
* **`lastValidBlockHeight`** (u64)：表示 `blockhash` 将过期的区块高度。引用此 `blockhash` 的交易在网络达到此 `lastValidBlockHeight` 之前有效。
...
* **区块哈希有效性：** 区块哈希在有限时间内有效，大约 2 分钟（尽管可能有所不同）。交易必须在其 `lastValidBlockHeight` 通过之前由网络确认。如果自上次获取以来已经过了相当长的时间，请始终获取新的区块哈希。
