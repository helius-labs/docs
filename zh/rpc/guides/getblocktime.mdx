...
[`getBlockTime`](https://www.helius.dev/docs/api-reference/rpc/http/getblocktime) RPC 方法提供指定区块的估计生产时间，该区块由其槽号标识。时间以 Unix 时间戳（自 Unix 纪元以来的秒数）返回。
...
* **`timestamp`** (i64)：估计的生成时间，以 Unix 时间戳表示（自 Unix 纪元以来的秒数）。
...
* **估计：** 时间是一个*估计值*。它是从验证者的投票时间戳的权益加权平均值得出的。虽然通常准确，但它不像区块哈希那样为每个区块提供保证的、加密安全的时间戳。
* **节点依赖性：** 可用性和精度可能会因查询的 RPC 节点而略有不同，尤其是对于非常近期（尚未最终确定）的区块。

本指南解释了如何使用`getBlockTime`来检索Solana网络上任何给定区块的预计生产时间戳。