...
[`getBlockHeight`](https://www.helius.dev/docs/api-reference/rpc/http/getblockheight) RPC 方法是一种查询 Solana 节点当前区块高度的简单方式。区块高度表示自创世区块（插槽 0）以来已处理的区块数量。
...
`getBlockHeight` 可以选择性地将配置对象作为其第一个参数（或不带参数调用以使用默认值）：
...
1.  `config`（对象，可选）：具有以下字段的配置对象：
    ...
    *   `minContextSlot`（数字，可选）：请求可以评估的最小插槽。这确保返回的区块高度来自等于或大于 `minContextSlot` 的插槽。
