...
description: 在交易从碎片中解码时订阅，以实现超低延迟监控，无需交易元数据。
---

<Warning>
**测试版功能 - 需要请求访问**

预处理交易订阅目前处于测试阶段。要请求访问，请通过[Telegram](https://t.me/helius_help)或[Discord](https://discord.gg/EqanqfQU)联系Helius团队。
</Warning>

预处理交易是从Solana接收交易数据的最快方式之一。LaserStream直接从到达验证器的碎片中解码交易，而不是等待完整的交易处理，使您比标准订阅提前几毫秒获取交易数据。
...
1. **碎片接收** → 验证器接收交易碎片（数据片段）
2. **碎片解码** → 碎片被解码为原始交易 ← **预处理交易在此可用**
3. **交易执行** → 交易由运行时执行
4. **元数据生成** → 计算前/后余额、日志和错误信息
5. **承诺** → 交易达到已处理/已确认/已完成状态

标准交易订阅在第5阶段提供数据 - 完整执行和元数据生成之后。预处理订阅在第2阶段提供 - 碎片解码后立即提供，在执行完成之前。
...
可以将预处理交易视为收到“提案”而没有“结果”。您可以看到用户尝试做什么，但看不到实际发生了什么。
...
JavaScript SDK 提供了一个专用的`subscribePreprocessed`函数，具有自动重新连接功能：
