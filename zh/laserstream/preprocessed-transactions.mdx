---
title: 预处理交易（测试版）
sidebarTitle: 预处理交易（测试版）
description: 流式传输 Solana 交易的最快方式。
---

<Warning>
**测试版功能 - 需要请求访问权限**

预处理交易目前处于测试阶段。要请求访问权限，请通过[Telegram](https://t.me/helius_help)或[Discord](https://discord.gg/EqanqfQU)联系 Helius 团队。
</Warning>

预处理交易是接收 Solana 交易的最快方式。LaserStream 直接从分片中解码交易，而不是等待完整的交易处理，这使您比标准订阅提前几毫秒获取交易数据。

本指南解释了何时使用预处理交易、可用的数据以及如何在所有 LaserStream SDK 中实现它们。

---

## 什么是预处理交易？

在 Solana 的架构中，交易在完全处理之前会经过几个阶段：

1. **分片接收** → 验证器接收交易分片（数据片段）
2. **分片解码** → 分片被解码为原始交易 ← **预处理交易在此可用**
3. **交易执行** → 交易由运行时执行
4. **元数据生成** → 计算预/后余额、日志和错误信息
5. **承诺** → 交易达到已处理/已确认/已完成状态

标准交易订阅在第 5 阶段提供数据 - 完整执行和元数据生成之后。预处理订阅在第 2 阶段提供 - 分片解码后立即提供，在执行完成之前。

**权衡：** 您可以提前几毫秒接收交易数据，但没有执行元数据，如余额变化、日志或错误信息。
...
<Info>
**重要：** 预处理交易的交付是尽力而为的，不是保证的。我们的目标是 99.99% 的交付率，但在以下情况下可能会丢失一些交易：

- 基础设施更新和重新部署
- 网络问题或验证器连接问题
- 分片解码或处理中的边缘情况

对于需要保证交付的关键应用程序，请改用标准[事务订阅](/zh/laserstream/guides/decoding-transaction-data)。
</Info>
...
可以将预处理的交易视为收到“提案”而没有“结果”。您可以看到用户尝试做什么，但看不到实际发生了什么。
...
JavaScript SDK提供了一个专用的`subscribePreprocessed`函数，具有自动重新连接功能：
...
**完整示例：** [preprocessed_transaction_sub.rs](https://github.com/helius-labs/laserstream-sdk/blob/main/rust/examples/preprocessed_transaction_sub.rs)
...
预处理订阅请求遵循与标准订阅类似的结构，但具有集中的过滤器集：
