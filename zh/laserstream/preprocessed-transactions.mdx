...
预处理交易是接收 Solana 交易的最快方式。LaserStream 直接从分片解码交易，而不是等待完整的交易处理，使您比标准订阅提前几毫秒获取交易数据。
...
<Info>
**重要：** 预处理交易的交付是尽力而为的，不是保证的。我们目标是 99.99% 的交付率，但在以下情况下可能会丢失一些交易：
