...
预处理交易是接收Solana交易的最快方式。LaserStream直接从到达验证器的shreds中解码交易，而不是等待完整的交易处理，使您比标准订阅提前几毫秒获取交易数据。
...
1. **Shred接收** → 验证器接收交易shreds（数据片段）
2. **Shred解码** → Shreds被解码为原始交易 ← **预处理交易在此可用**
3. **交易执行** → 交易由运行时执行
4. **元数据生成** → 计算前/后余额、日志和错误信息
5. **承诺** → 交易达到已处理/已确认/已完成状态

标准交易订阅在第5阶段提供数据 - 完整执行和元数据生成之后。预处理订阅在第2阶段提供 - 在解码shreds后立即提供，在执行完成之前。
...
对于需要保证交付的关键应用程序，请改用标准[交易订阅](/zh/laserstream/guides/decoding-transaction-data)。
</Info>
