...
description: 最快的方式来流式传输Solana交易。
---

<Warning>
**测试版功能 - 需要请求访问**

预处理交易目前处于测试阶段。要请求访问，请通过[Telegram](https://t.me/helius_help)或[Discord](https://discord.gg/EqanqfQU)联系Helius团队。
</Warning>

预处理交易是接收Solana交易的最快方式。LaserStream直接从分片解码交易，当它们到达验证器时，您可以比标准订阅提前几毫秒访问交易数据，而无需等待完整的交易处理。

本指南解释了何时使用预处理交易、可用的数据以及如何在所有LaserStream SDK中实现它们。

---

## 什么是预处理交易？

在Solana的架构中，交易在完全处理之前会经过几个阶段：
...
标准交易订阅在第5阶段提供数据 - 完整执行和元数据生成之后。预处理订阅在第2阶段提供 - 分片解码后立即提供，在执行完成之前。
...
<Info>
**重要：** 预处理交易的交付是尽力而为的，不是保证的。我们的目标是99.99%的交付率，但在以下情况下可能会丢失一些交易：
