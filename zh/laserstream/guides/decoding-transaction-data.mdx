...
如果 `versioned` 是 `true`，则 `addressTableLookups` 会出现一个链上表和两个索引列表。查找表将地址数量的硬性上限提升到几十个，同时保持数据包在 1,232 字节的 MTU 之内。
...
- **账户密钥查找**：指令通过 `accountKeys` 数组中的索引引用账户
