...
title: 解码和解析交易数据
sidebarTitle: 解码交易数据
description: 了解如何解码和解析来自Laserstream的交易数据，以更好地理解Solana交易。
---
...
**挑战：** 原始交易数据以二进制字节数组的形式出现，如 `<Buffer 00 bf a0 e8...>`，而不是可读的地址和签名。
...
您的控制台现在显示一个包装器—`filters`, `createdAt` 加上一个 `transaction` 分支，隐藏了两个子项：

* `transaction.transaction.transaction` → 签名的**消息**
* `transaction.transaction.meta` → 执行的**元数据**
...
目前，所有看起来像 `Uint8Array` 的内容仍然是不透明的。
...
**为什么要解码？** 原始 Laserstream 数据包含签名、账户密钥和哈希，以二进制 `Uint8Array` 对象形式存在，不可读。您需要将这些转换为 base58 字符串，以便理解交易。
...
用户创建一个消息，指定*什么*，*谁*以及*何时为止*。以下是如何解码每个部分：
...
`accountKeys` 是一个公共密钥的简单列表，作为查找表。交易中的每个后续整数 - `programIdIndex`，每个指令的 `accounts` 数组中的元素 - 通过索引指回此列表，每条消息节省超过一千字节。
...
`recentBlockhash` 一旦滚动出最后 150 个区块哈希后就会过期，在主网上大约是九十秒。
...
- **程序 ID** (`programIdIndex`): 指向 `accountKeys` 数组中的一个地址（例如，索引 10 = `ComputeBudget111111111111111111111111111111`）
- **账户** (`accounts`): 一个 base58 编码的字符串，表示此指令涉及的账户索引
- **数据** (`data`): 实际的指令数据，编码为 base58

由于 `convertBuffers` 函数，账户显示为 base58，但实际上包含账户索引（例如，`"3vtmrQMafzDoG2CBz1iqgXPTnC"` 解码为索引 [21, 19, 12, 17, 2, 6, 1, 22]）
...
`signatures` 包含证明所需账户授权此交易的加密签名。签名数量必须与 `header.numRequiredSignatures` 匹配。
...
如果 `versioned` 是 `true`，`addressTableLookups` 将与链上表和两个索引列表一起出现。查找表将地址数量的硬性上限提升到几十个，同时保持数据包在 1,232 字节的 MTU 之内。

### 如何连接：流程

从基本原理来看，以下是发生的事情：

1. **构建查找表**：`accountKeys` 列出此交易将涉及的所有地址
2. **设置规则**：`header` 指定需要多少个签名以及哪些账户是只读的
...
5. **设置过期时间**：`recentBlockhash` 确保此交易不能在以后重放
...
余额数组按索引对应于 `accountKeys` 数组：
- 账户 0：损失 15000 lamports（手续费）
...
内部指令是程序在执行期间调用的附加指令。它们不是原始交易的一部分，但由主要指令触发。
...
日志消息提供程序执行的时间顺序跟踪，显示调用了哪些程序以及它们输出的任何自定义日志消息。
...
- **两部分结构**：每个交易都有一个**消息**（请求的内容）和**元数据**（实际发生的内容）
- **二进制解码**：使用`bs58.encode()`将二进制字段转换为可读的base58字符串
- **账户密钥查找**：指令通过索引在`accountKeys`数组中引用账户
