...
**为什么解码？** 原始 Laserstream 数据包含签名、账户密钥和哈希，以二进制 `Uint8Array` 对象形式存在，不可读。您需要将这些转换为 base58 字符串，以便理解交易。
...
`accountKeys` 是一个公共密钥的简单列表，作为查找表。交易中的每个后续整数 - `programIdIndex`，指令的每个 `accounts` 数组中的元素 - 通过索引指回此列表，每条消息节省超过一千字节。
...
如果 `versioned` 是 `true`，`addressTableLookups` 将与链上表和两个索引列表一起出现。查找表将地址数量的硬上限提升到几十个，同时保持数据包在 1,232 字节的 MTU 之内。
...
- **双部分结构**：每个交易都有一个**消息**（请求的内容）和**元数据**（实际发生的事情）
