...
现在我们知道铸币地址位于字节0-31，我们可以更具体一些。假设我们只想监控USDC代币账户。我们可以将我们的 `datasize` 过滤器与 `memcmp` 过滤器结合，以定位确切的铸币地址：
...
除了我们使用的基本 `owner`、`datasize` 和 `memcmp` 过滤器之外，账户订阅还支持其他过滤选项以进一步缩小结果范围：
