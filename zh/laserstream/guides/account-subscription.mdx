...
**刚刚发生了什么？** 我们的订阅完美运行！我们请求Laserstream通知我们关于代币账户的更改，它提供了关于账户 `BKMHWYLAX4un3HUbR7a3u9jPmzCiLNa4mSj1RiX11eWF` 的更新。

此账户具有：
- **2,039,280 lamports**（约0.002 SOL余额 - 这是此代币账户的免租金额）
- **所有者程序** `TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`（这是SPL Token程序）
- **交易签名** `5C9Hr5nG2j8eQz6inxPmfyjbYdmXddzUDyR1iQgEnjYQ3RNvuP4Zzc8t1enLNy7Rk8KNCtQPEQztENYWxkt9GaVD` 显示导致此账户更改的具体交易
...
这不是魔法 - 它来自 [SPL Token 程序的账户结构](https://github.com/solana-program/token/blob/d05d10807fe8cf157f6e1f024c708274c30c953a/program/src/state.rs#L87)。查看源代码，我们可以看到 `Account` 结构体定义了正好 165 字节：
...
现在我们知道铸币地址位于字节0-31，我们可以更加具体。假设我们只想监控USDC代币账户。我们可以将我们的`datasize`过滤器与`memcmp`过滤器结合，以定位确切的铸币地址：
...
原始账户更新包含我们需要解码的二进制数据。由于 Solana 使用 base58 编码地址和签名，我们使用 `bs58.encode()` 函数将二进制 Buffer 对象转换为可读字符串。
...
1. **广撒网** 使用 `owner` - “给我所有由这个程序管理的账户”
2. **按结构过滤** 使用 `datasize` - “但仅限于这种特定类型的账户”
3. **定位特定数据** 使用 `memcmp` - “并且仅限于包含此特定信息的账户”
4. **监控已知账户** 使用 `account` - “或者只监控我关心的这些确切账户”
