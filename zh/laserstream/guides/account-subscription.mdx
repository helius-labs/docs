...
**刚刚发生了什么？** 我们的订阅完美地工作了！我们请求Laserstream通知我们关于代币账户的变化，它提供了关于账户 `BKMHWYLAX4un3HUbR7a3u9jPmzCiLNa4mSj1RiX11eWF` 的更新。

该账户有：
- **2,039,280 lamports**（约0.002 SOL余额 - 这是该代币账户的免租金额）
- **所有者程序** `TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`（这是SPL Token程序）
- **交易签名** `5C9Hr5nG2j8eQz6inxPmfyjbYdmXddzUDyR1iQgEnjYQ3RNvuP4Zzc8t1enLNy7Rk8KNCtQPEQztENYWxkt9GaVD` 显示了导致该账户变化的具体交易
...
现在我们知道铸币地址位于字节0-31，我们可以更具体。假设我们只想监控USDC代币账户。我们可以将我们的 `datasize` 过滤器与一个 `memcmp` 过滤器结合，以定位确切的铸币地址：
...
原始账户更新包含我们需要解码的二进制数据。由于Solana使用base58编码地址和签名，我们使用 `bs58.encode()` 函数将二进制缓冲对象转换为可读字符串。
...
除了我们使用的基本 `owner`、`datasize` 和 `memcmp` 过滤器之外，账户订阅还支持其他过滤选项，以进一步缩小您的结果范围：
