...
_（你只需要 `ws` 库用于 Node.js WebSocket 客户端。不需要额外的库。）_
...
将占位符 **`laserstream-ws-url`** 替换为你的实际 WebSocket URL，并按如下方式包含你的 `api-key`：
...
每当 **确认** 的代币交易涉及 `TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA` 时，你将开始接收交易事件。预期会收到显示交易详情和槽位的 JSON 对象。

***

### Transaction Subscribe

transactionSubscribe websocket 方法启用实时交易事件。要使用它，请提供一个 `TransactionSubscribeFilter`，并可选地包括 `TransactionSubscribeOptions` 以进行进一步定制。
...
`accountInclude`：你希望接收交易更新的账户列表。这意味着只有一个账户必须包含在交易更新中（例如，账户 1 或账户 2）。

`accountExclude`：你希望从交易更新中排除的账户列表。

`accountRequired`：交易必须涉及这些指定的账户才能包含在更新中。这意味着所有账户都必须包含在交易更新中（例如，账户 1 和账户 2）。
...
`fromSlot`：从特定的Solana插槽开始订阅，从该插槽向前重播任何相关交易，直到您赶上实时数据。
...
在此示例中，我们订阅包含Raydium账户的交易 `675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8`。每当发生包含`675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8`的交易时，我们将在交易的`accountKeys`中收到一个websocket通知。
...
* `encoding`：指定AccountNotification中返回数据的格式。支持的值：**base58**、**base64**、**base64+zstd**、**jsonParsed**（默认是**base58**）。
* `commitment`：定义交易的承诺级别。支持的值：**finalized**、**confirmed**、**processed**（默认是**finalized**）。
* `fromSlot`：从此插槽重播，接收账户的历史更改直到当前插槽。
...
\
每当账户数据或此账户的 lamports 发生更改时，我们将看到更新。由于 `slot` 和 `unixTimestamp` 都是返回账户数据的一部分，因此此特定账户会在频繁的间隔内发生这种情况。
