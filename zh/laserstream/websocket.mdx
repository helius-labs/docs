...
`accountInclude`：你希望接收交易更新的账户列表。这意味着只有一个账户必须包含在交易更新中（例如，账户1或账户2）。
...
在此示例中，我们订阅包含Raydium账户的交易 `675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8`。每当发生包含`675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8`的交易时，我们将收到一个websocket通知。
...
\
每当账户数据或此账户的 lamports 发生变化时，我们将看到更新。由于 `slot` 和 `unixTimestamp` 都是返回账户数据的一部分，因此这种情况会在此特定账户上频繁发生。
...
Atlas 增强型 WebSockets 将在底层由 LaserStream **驱动**。这意味着您可以继续使用相同的 Atlas 端点，并且仍然可以访问 LaserStream 的高级功能，例如 **`fromSlot`** 重播，而无需迁移。如果您已经在使用 Atlas，**无需更改**；如果您更喜欢专用的 LaserStream 端点，也可以使用它们。
