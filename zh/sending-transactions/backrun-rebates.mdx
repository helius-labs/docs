...
<Note>
**这是一个可选功能。** Helius 在没有您明确许可的情况下绝不会参与回跑返利。您必须通过在交易请求中添加 `rebate-address` 参数来选择加入。
</Note>
...
<Steps titleSize="h3">
  ...
  <Step title="拍卖过程">
    当您的交易通过标准 RPC 路径处理时，Helius 还会将未签名的副本转发到我们的私人订单流拍卖中，经过预批准的、通过 KYC 的搜索者会竞标回跑您的交易
  </Step>
  <Step title="捆绑创建">
    最高出价者获胜并创建一个捆绑包：**[您的交易，搜索者交易]**。您的交易总是首先执行
  </Step>
  <Step title="自动支付">
    如果捆绑包上链，Helius 会自动将商定的返利直接支付到您指定的 SOL 地址
  </Step>
</Steps>
