...
此示例将 SOL 转移到您选择的账户。它使用 `sendSmartTransaction` 发送一个不跳过预检的优化交易：
...
通过[`getLatestBlockhash`](/zh/rpc/guides/getlatestblockhash)获取当前区块哈希是一个常见模式，存储`lastValidBlockHeight`，并在区块哈希过期前重试交易。
