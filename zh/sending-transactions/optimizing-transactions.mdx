...
1. 使用[质押连接](https://www.helius.dev/staked-connections)
2. 使用专门的着陆服务，如 [Sender](https://www.helius.dev/sender)

本文涵盖使用质押连接的交易优化最佳实践，这是所有 Helius 付费计划的默认方法。

质押连接最适合于延迟对您的业务不关键的用例（例如，支付、钱包、社交应用等）

如果您是高级交易员（例如，高频交易、MEV 搜索者、套利者、代币狙击手等）并寻找专门的超低延迟交易着陆服务，请阅读我们的 [Sender 教程](/zh/sending-transactions/sender)。

## 摘要
Helius 的质押连接保证 100% 的交易交付，并具有最小的确认时间。为了优化使用质押连接的交易着陆率，我们建议以下最佳实践：
...
<Tip>
  想要更深入了解？我们在这篇[博客文章](https://www.helius.dev/blog/how-to-land-transactions-on-solana)中涵盖了所有基础知识。
</Tip>
...
然而，如果您使用质押连接并希望优化您的设置以获得尽可能低的延迟，我们建议以下优化（除了应用上述最佳实践）：

- 您的客户端服务器（用于发送交易的机器）应位于美国东部或西欧。
- 如果您希望与 Helius 交易发送服务器共址，请选择 FRA 或 PIT。
...
然后，创建一个新的版本化交易并签名——这是下一步模拟交易时所必需的，因为交易必须签名。
...
这一步几乎是第一步的重复。然而，初始指令数组已被更改，以添加两个指令来最佳地设置计算单元限制和价格。

现在，发送交易。

无论是否进行预检或更改其他发送选项，交易都会通过我们为所有付费计划提供的抵押连接进行路由。

### 轮询交易状态和重新广播

<Warning> 虽然抵押连接会将交易直接转发给领导者，但交易仍有可能在[银行阶段](https://www.helius.dev/blog/solana-virtual-machine#the-banking-stage)被丢弃。建议用户使用自己的重新广播逻辑，而不是依赖RPC为他们重试交易。 </Warning>
...
只有在区块哈希不再有效时重新签署交易才是至关重要的，否则网络可能会接受两个交易。
...
`sendSmartTransaction`方法有一个60秒的超时时间。由于区块哈希在150个槽内有效，并假设完美的400ms槽，我们可以合理地假设交易的区块哈希将在一分钟后失效。
