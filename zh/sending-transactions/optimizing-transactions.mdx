...
- 如果您希望与Helius交易发送服务器共址，请选择FRA或PIT。
- 避免从远离验证器网络的地区发送（例如，拉丁美洲、南非）。
- 预热Helius区域缓存以最小化尾部延迟。
- 每个区域只需要一个预热线程——再多也没有任何好处。
- 每秒使用您用于发送交易的相同端点和API密钥发送一个[`getHealth`](/zh/rpc/guides/gethealth) RPC调用。
...
`sendSmartTransaction`方法在我们的[Helius Node.js SDK](/zh/sdks)中可用，适用于[版本\>= 1.3.2](https://www.npmjs.com/package/helius-sdk)。要更新到更新版本的SDK，请运行`npm update helius-sdk`。

此示例将SOL转移到您选择的账户。它使用`sendSmartTransaction`发送一个不跳过预检的优化交易：
...
`send_smart_transaction`方法在我们的[Rust SDK](/zh/sdks)中可用，适用于[版本\>= 0.1.5](https://crates.io/crates/helius)。要更新到更新版本的SDK，请运行`cargo update helius`。

以下示例将 0.01 SOL 转移到您选择的账户。

它利用`send_smart_transaction`发送一个跳过预检并在必要时重试两次的优化交易：
...
首先，为了序列化交易，Transaction和VersionedTransaction类型都有一个`.serialize()`方法。然后使用[bs58包](https://www.npmjs.com/package/bs58)对交易进行编码。

你的代码应类似于`bs58.encode(txt.serialize());`
...
通过[`getLatestBlockhash`](/zh/rpc/guides/getlatestblockhash)获取当前区块哈希，存储`lastValidBlockHeight`，并在区块哈希过期前重试交易是一个常见模式。
...
@solana/web3.js SDK在其`Connection`类上也有一个`getSignatureStatuses`方法来获取多个签名的当前状态。

### sendSmartTransaction如何处理轮询和重新广播

`sendSmartTransaction`方法的超时时间为60秒。由于区块哈希在150个槽内有效，并假设完美的400毫秒槽，我们可以合理地假设交易的区块哈希将在一分钟后失效。
...
然后，该方法使用 `pollTransactionConfirmation()` 方法轮询交易的确认状态。此方法每五秒检查一次交易状态，最多检查三次。
