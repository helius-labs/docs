...
参数`skipPreflight` **必须**设置为`true`。发送器针对优先考虑速度而非交易验证的交易者进行了优化。
...
<Info>
本节仅在您获得增加的 TPS 限制并从 Helius 团队收到 API 密钥时相关。标准用户可以跳过本节。
</Info>
...
* 通过[Helius Dashboard](https://dashboard.helius.dev)联系支持
