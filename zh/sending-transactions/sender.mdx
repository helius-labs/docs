...
<Card title="API 参考" icon="code" href="/zh/api-reference/sender/sendtransaction">
  查看完整的 Sender API 参考，包括请求/响应模式和参数。
</Card>
...
高级示例通过动态 Jito 提示（第 75 百分位）、自动计算单元计算、动态优先费用和重试逻辑改进了简单版本。
...
- **默认速率限制**：每秒 15 笔交易
- **无信用使用**：Sender 交易不消耗您计划中的 API 信用
- **专业计划升级**：专业计划用户可以请求显著的速率限制增加，以支持高吞吐量交易应用程序

### 使用 API 密钥认证的自定义 TPS

<Info>
本节仅在您获得增加的 TPS 限制并从 Helius 团队收到 API 密钥时相关。标准用户可以跳过此部分。
</Info>
