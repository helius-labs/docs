...
<CardGroup cols={2}>
  <Card title="默认双路由" icon="arrows-split-up-and-left">
    同时将交易发送到 Solana 验证器和 Jito 基础设施，以最大化包含概率。需要最低 0.0002 SOL 小费。
  </Card>
  <Card title="仅限 SWQOS 的替代方案" icon="dollar-sign">
...
- **优先费用**：通过 `ComputeBudgetProgram.setComputeUnitPrice` 计算单元定价，以在验证器队列中优先处理您的交易
...
发送器端点使用与标准 RPC 端点相同的 `sendTransaction` 方法，但对最佳性能有特定要求。**所有交易必须包括小费和优先费用，并跳过预检检查。**
...
<Warning>
上述 `BASE64_ENCODED_TRANSACTION` 必须包含一个最低小费的 SOL 转账指令到指定的小费账户和一个计算单元价格指令。没有这两个要求，您的交易将被拒绝。
</Warning>

<Card title="API 参考" icon="code" href="/zh/api-reference/sender/sendtransaction">
  查看完整的发送器 API 参考，包括请求/响应模式和参数。
</Card>
...
高级示例在简单版本的基础上进行了改进，具有动态 Jito 小费（第 75 百分位）、自动计算单元计算、动态优先费用和重试逻辑。
