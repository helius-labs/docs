...
<Card title="API参考" icon="code" href="/zh/api-reference/sender/sendtransaction">
  查看完整的Sender API参考，包括请求/响应模式和参数。
</Card>
...
高级示例通过动态Jito提示（第75百分位）、自动计算单元计算、动态优先费用和重试逻辑改进了简单版本。
...
### 端点选择
- **前端应用程序**：使用`https://sender.helius-rpc.com/fast`以避免CORS预检失败。此端点会自动路由到最近的位置以获得最佳性能。
- **后端应用程序**：选择最接近您基础设施的区域HTTP [端点](#endpoints)以获得最佳性能

### 连接预热
- 在空闲时间超过1分钟时使用`/ping`端点
- 实施定期ping调用（每30-60秒）以保持连接预热
- 在高频交易会话开始前预热连接

### 交易设置
- 使用`skipPreflight: true`和`maxRetries: 0`
- 实施您自己的重试逻辑
- 向指定账户至少包含0.0002 SOL的小费
...
- **默认速率限制**：每秒15笔交易
- **无信用使用**：Sender交易不消耗您计划中的API信用
- **专业计划升级**：专业计划用户可以请求显著的速率限制增加以支持高吞吐量交易应用程序

### 使用API密钥认证的自定义TPS

<Info>
本节仅在您获得增加的TPS限制并从Helius团队收到API密钥时相关。标准用户可以跳过本节。
</Info>
...
<Note>
还没有自定义 TPS 限制？如果您的应用程序需要超过 15 TPS，请[在此请求更高的限制](https://form.typeform.com/to/FuvpgSNt)。
</Note>
...
<Card title="请求更高的 TPS 限制" icon="rocket" href="https://form.typeform.com/to/FuvpgSNt">
...
* 加入我们的 [Discord 社区](https://discord.com/invite/6GXdee3gBj) 以获得实时帮助
