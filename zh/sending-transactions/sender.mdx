...
<CardGroup cols={2}>
  ...
  <Card title="仅SWQOS的替代方案" icon="dollar-sign">
    为了优化成本的交易，在任何端点URL中添加`?swqos_only=true`。仅通过SWQOS基础设施路由，最低小费要求为0.000005 SOL。
  </Card>
</CardGroup>
...
<Warning>
上述`BASE64_ENCODED_TRANSACTION`必须包含一个带有最低小费的SOL转账指令到指定的小费账户和一个计算单元价格指令。没有这两个要求，您的交易将被拒绝。
</Warning>
