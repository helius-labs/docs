...
<CardGroup cols={2}>
  ...
  <Card title="高吞吐量" icon="gauge-high" href="https://form.typeform.com/to/FuvpgSNt">
...
<CardGroup cols={2}>
  <Card title="默认双路由" icon="arrows-split-up-and-left">
    同时将交易发送到Solana验证者和Jito基础设施，以最大化包含概率。需要最低0.0002 SOL的小费。
  </Card>
  <Card title="仅SWQOS替代方案" icon="dollar-sign">
    对于成本优化的交易，在任何端点URL中添加`?swqos_only=true`。仅通过SWQOS基础设施路由，最低小费要求为0.000005 SOL。
  </Card>
</CardGroup>
...
必须将`skipPreflight`参数设置为`true`。发送器针对优先考虑速度而非交易验证的交易者进行了优化。
...
- **小费**：最低 0.0002 SOL 转账到指定的小费账户（或仅 SWQOS 为 0.000005 SOL）
...
**Jito 小费**：参与拍卖的最低要求是 0.0002 SOL。有关当前最佳实践小费大小，请参阅 [Jito 小费指南](https://docs.jito.wtf/lowlatencytxnsend/#tips)。
...
发送器端点使用与标准RPC端点相同的`sendTransaction`方法，但对最佳性能有特定要求。**所有交易必须包括小费和优先费用，并跳过预检检查。**
...
<Warning>
上述`BASE64_ENCODED_TRANSACTION`必须包含一个最低小费的SOL转账指令到指定的小费账户和一个计算单元价格指令。没有这两个要求，您的交易将被拒绝。
</Warning>

<Card title="API参考" icon="code" href="/zh/api-reference/sender/sendtransaction">
  查看完整的发送器API参考，包括请求/响应模式和参数。
</Card>
...
高级示例在简单版本的基础上进行了改进，具有动态Jito小费（第75百分位）、自动计算单元计算、动态优先费用和重试逻辑。
...
- **默认速率限制**：每秒 15 笔交易
- **无信用使用**：发送者交易不会消耗您计划中的 API 信用
- **专业计划升级**：专业计划用户可以请求显著的速率限制增加以支持高吞吐量交易应用程序

### 使用 API 密钥认证的自定义 TPS

<Info>
如果您已获得增加的 TPS 限制并从 Helius 团队收到 API 密钥，则此部分仅相关。标准用户可以跳过此部分。
</Info>

如果您已获得更高的 TPS 限制批准，您将收到一个专用的 API 密钥用于 Sender。要使用它，请将 API 密钥作为查询参数附加到您的端点 URL：
...
<Note>
还没有自定义TPS限制？如果您的应用程序需要超过15 TPS，请[在此请求更高的限制](https://form.typeform.com/to/FuvpgSNt)。
</Note>
...
<Card title="请求更高的TPS限制" icon="rocket" href="https://form.typeform.com/to/FuvpgSNt">
需要超过15 TPS？请填写我们的请求表格。专业计划用户可以获得高频交易应用程序的显著速率限制升级。
</Card>

其他支持选项：

* 通过[Helius仪表板](https://dashboard.helius.dev)联系支持
* 加入我们的[Discord社区](https://discord.com/invite/6GXdee3gBj)以获得实时帮助
