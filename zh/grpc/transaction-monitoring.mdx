---
title: 使用 Yellowstone gRPC 进行交易监控
sidebarTitle: 交易监控
description: 通过程序过滤、执行细节和代币余额变化跟踪，实时流式传输 Solana 交易。
---
...
<Info>
**先决条件：** 本指南假设您已完成 [Yellowstone gRPC 快速入门](/grpc/quickstart) 并已设置好工作流。
</Info>
...
<Tabs>
  ...
  <Tab title="高级过滤">
    **结合多个过滤条件**
    ...
    <Warning>
    **过滤逻辑：** accountInclude (或) AND accountRequired (与) AND NOT accountExclude
    </Warning>
  </Tab>
</Tabs>
...
<CardGroup cols={2}>
  <Card title="包含逻辑 (OR)" icon="plus">
    **accountInclude:** 交易必须涉及这些账户中的任意一个
    
    **示例：** `["A", "B"]` 匹配涉及账户A或账户B的交易
  </Card>
  
  <Card title="必需逻辑 (AND)" icon="check">
    **accountRequired:** 交易必须涉及所有这些账户
    
    **示例：** `["A", "B"]` 匹配涉及账户A和账户B的交易
  </Card>
  
  <Card title="排除逻辑 (NOT)" icon="minus">
    **accountExclude:** 交易不得涉及任何这些账户
    
    **示例：** `["A", "B"]` 排除涉及账户A或账户B的交易
  </Card>
...
<CardGroup cols={2}>
  <Card title="插槽和区块监控" icon="cube" href="/grpc/slot-and-block-monitoring">
    学习监控网络共识和区块生产
  </Card>
  <Card title="高级模式" icon="chart-line" href="/grpc/stream-pump-amm-data">
    真实案例：监控 Pump AMM 数据
  </Card>
</CardGroup>
