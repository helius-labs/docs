  ...
  <Step title="获取您的凭据">
    ...
    <Tabs>
      <Tab title="LaserStream">
        1. 在 [dashboard.helius.dev](https://dashboard.helius.dev) 上注册 Developer+ 计划（devnet）或 Professional 计划（mainnet）
        2. 从仪表板获取您的 API 密钥
        3. 选择您的区域端点：
      ...
      <Tab title="Dedicated Nodes">
        1. 从 [dashboard.helius.dev](https://dashboard.helius.dev/dedicated-nodes) 订购一个专用节点
        2. 一旦配置完成，您将收到：
...
<CardGroup cols={2}>
  ...
  <Card title="流泵AMM数据" icon="chart-line" href="/zh/grpc/stream-pump-amm-data">
    真实世界示例：监控DeFi协议数据
  </Card>
</CardGroup>
