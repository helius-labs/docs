...
<Note>
**快速参考**：DAS API 为所有 Solana 资产提供统一接口。使用 `getAsset` 处理单个资产，使用 `getAssetsByOwner` 处理钱包持有，使用 `searchAssets` 进行过滤查询，以及用于压缩 NFT 的专用方法。
</Note>
...
<CardGroup cols={3}>
  <Card title="单个资产" icon="cube" href="#api方法">
    获取特定资产的详细数据
  </Card>
  <Card title="资产集合" icon="layer-group" href="#api方法">
    按所有者、创作者或集合获取资产
  </Card>
  <Card title="高级查询" icon="magnifying-glass" href="#api方法">
    使用复杂条件搜索和过滤资产
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="可替代代币" icon="coins" href="#处理特殊资产类型">
    访问 SPL 代币并获取价格数据
  </Card>
  <Card title="压缩 NFT" icon="file-zipper" href="#处理特殊资产类型">
    处理状态压缩资产
  </Card>
  <Card title="铭文" icon="file-signature" href="#处理特殊资产类型">
    访问铭文和 SPL-20 数据
  </Card>
  <Card title="链下数据" icon="cloud" href="#处理特殊资产类型">
...
<Info>
  Helius 已扩展 DAS API 以支持**所有代币**，包括普通 SPL 代币（无元数据）和 Token-2022（及其扩展）。可通过 `getAssetsByOwner` 和 `searchAssets` 方法获得可替代代币支持。
</Info>
