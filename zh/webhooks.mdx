...
* **增强型交易 Webhooks**：提供特定交易类型（例如，NFT 销售）的可读解析数据，相关于您监控的地址。如果您需要过滤的、可操作的见解，这是理想的选择。
* **原始交易 Webhooks**：此选项提供与您监控的地址相关的所有交易的原始交易数据。它不允许按交易类型进行过滤。
...
如果您更喜欢无代码解决方案，并且需要查看日志和发送测试 webhook 事件等附加功能，Helius UI 是完美的选择。您可以通过我们的 [仪表板](https://dashboard.helius.dev/webhooks) 直接访问它。您可以通过仪表板添加最多 25 个地址。要监控超过 25 个地址，您可以使用我们的 API 或 SDK。
...
这些 SDK 提供强大的抽象，增强了 webhooks 的功能。
