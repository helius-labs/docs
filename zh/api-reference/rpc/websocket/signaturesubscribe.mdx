...
og:title: signatureSubscribe Solana RPC 方法
sidebarTitle: signatureSubscribe
description: 订阅以在具有给定签名的交易达到指定的承诺级别时接收通知。
og:description: 获取免费的 RPC 并探索 signatureSubscribe RPC 方法。包括请求参数、响应示例、代码片段以及
  Solana 主网和 Devnet 的浏览器内测试。
---

这是对单个通知的订阅。一旦通知 `signatureNotification` 被 RPC 发送，服务器会自动取消订阅。
...
<Note>Websockets 有一个 10 分钟的非活动计时器；强烈建议实施健康检查并每分钟发送 ping 以保持 websocket 连接活跃。</Note>
...
<ParamField body="params" type="array" required>
    <Expandable title="属性" defaultOpen>
        <ParamField body="signature" type="string" required>
            ...
            交易签名必须是交易的第一个签名（有关详细信息，请参阅交易 ID）。
        </ParamField>
...
- `value: <object|string>` - RpcSignatureResult 的通知值，结果为：
  - 当 `enableReceivedNotification` 是 `true` 并且签名已接收时：字面字符串 `"receivedSignature"`，或
  - 当签名被处理时：`err: <object|null>`：
    - 如果交易在指定的承诺级别成功处理，则为 `null`，或
