...
og:title: logsUnsubscribe Solana RPC 方法
sidebarTitle: logsUnsubscribe
description: 取消订阅交易日志。
og:description: 获取免费的 RPC 并探索 logsUnsubscribe RPC 方法。包括请求参数、响应示例、代码片段，以及 Solana
  主网和 Devnet 的浏览器内测试。
---
...
<Note>Websockets 有 10 分钟的非活动计时器；强烈建议实施健康检查并每分钟发送 ping 以保持 websocket 连接活跃。</Note>
