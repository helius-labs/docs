...
[**增强 WebSockets**](/enhanced-websockets)
...
[**Webhooks**](/webhooks) 允许 Helius 在链上事件发生时通过 HTTP POST 通知您的服务器。非常适合**不需要**持久连接的情况。

1. 通过 [API](/api-reference/webhooks/create-webhook)、[SDK](/sdks) 或 [Dashboard](https://dashboard.helius.dev) **创建** webhook。
...
[**gRPC**](/grpc) 是最灵活且高性能的事件监听解决方案，可通过 [**LaserStream**](/laserstream) 或 [**Dedicated Nodes**](/dedicated-nodes) 使用。
...
1. Helius 的解析交易 API
2. 常规 Solana RPC 调用：`getSignaturesForAddress` + `getParsedTransaction` 或 `getTransaction`
...
**4.1.1 单个或批量交易（**[**`/v0/transactions`**](/api-reference/enhanced-transactions/gettransactions)**)**
...
**4.1.2 按地址解析交易（**[**`/v0/addresses/{address}/transactions`**](/api-reference/enhanced-transactions/gettransactionsbyaddress)**)**
...
2. **`getParsedTransaction`**：返回给定签名的**Solana解析**JSON。
3. **`getTransaction`**：返回**原始**二进制编码的交易，您可以使用外部库（例如，[Blockbuster](https://github.com/helius-labs/blockbuster)）进行解析，如果您需要专门的cNFT解码。

**4.2.1 `getSignaturesForAddress`**

这是一种**支持分页**的方法。您可以传递`before`或`until`参数。
...
查找引用**cNFT**程序或**Merkle树**的指令。如果您使用`getTransaction`，您将获得原始数据（例如，base64），您需要使用专门的解析器进行解码。
...
2. **检索历史数据**
   ...
   * **常规RPC**：`getSignaturesForAddress` + `getParsedTransaction`（或`getTransaction` + 手动解析）以获得最大灵活性，或如果您已经依赖于标准Solana RPC调用。
3. **构建完整时间线**
   * 合并**未来**（实时）事件与**过去**（历史）数据。
...
* **分页**: 对于活动频繁的地址，您可能需要使用 `before` 或 `until` 进行迭代以获取较旧的数据。
