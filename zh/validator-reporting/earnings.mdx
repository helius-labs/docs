...
<Tip>
**最佳实践**：使用 `30d` 期间进行月度报告，使用 `7d` 进行每周绩效评估。对于详细分析，使用自定义 epoch 范围与 `start_epoch` 和 `end_epoch`。
</Tip>

<Warning>
**数据可用性**：历史数据从 epoch 790 开始。查询更早的 epoch 将返回 400 错误。
</Warning>
