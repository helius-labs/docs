...
检索质押账户列表的质押奖励（通胀）和MEV奖励。此端点最多接受100个质押账户或投票账户的公钥，并返回每个账户在指定时间段内的聚合奖励指标。
...
<ResponseField name="vote_accounts" type="array">
  直接查询的投票账户公钥的指标（通过具有MEV奖励但无委托信息来识别）
...
<Tip>
**批量处理**：在查询多个账户时，考虑将它们分组为 100 个以内，以保持在请求限制内，同时最小化 API 调用次数。
</Tip>

<Warning>
**账户类型**：如果不确定一个地址是质押账户还是投票账户，可以查询它；API 会在响应中自动正确分类。
</Warning>
