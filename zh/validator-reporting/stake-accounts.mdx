    ...
    <ResponseField name="rewards" type="object">
      ...
      <Expandable title="rewards 属性">
        ...
        <ResponseField name="mev_rewards_lamports" type="integer">
          来自 Jito 提示分配的以 lamports 为单位的总 MEV 奖励
        </ResponseField>

        <ResponseField name="mev_rewards_sol" type="number">
          来自 Jito 提示分配的以 SOL 为单位的总 MEV 奖励
        </ResponseField>
...
<CardGroup cols={2}>
  <Card title="质押账户" icon="hand-holding-dollar">
    具有委托信息的账户被分类为质押账户。这些账户既有质押账户地址，也有它们被委托到的投票账户。
  </Card>
...
<Note>
**账户限制**：每个请求最多 100 个账户。对于更大的批次，请拆分为多个请求。
</Note>
...
<Tip>
**批处理**：在查询多个账户时，考虑将它们分组为 100 个以内，以在请求限制内最大限度地减少 API 调用。
</Tip>
