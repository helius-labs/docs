...
请注意，优先费用（即区块奖励）目前归于验证者，因为 SIMD-123 尚未上线，协议内没有与委托者共享的方式。
...
<ResponseField name="calculation_basis" type="object">
  ...
  <Expandable title="calculation_basis properties">
    ...
    <ResponseField name="warning" type="string" nullable>
      当部分周期被排除在计算之外时的警告信息（例如，"1个部分周期被排除在APY计算之外（当前周期的质押奖励不完整）"）
    </ResponseField>
...
在计算APY时，API会自动排除部分周期（即，具有不完整质押奖励数据的周期）以确保计算的准确性。

<Note>
**工作原理：**
- `epochs_in_range`：您请求范围内的总周期数（例如，3个周期）
- `epochs_covered`：用于APY计算的完整周期数（例如，2个周期）
- `epochs_partial`：排除的部分周期数（例如，1个周期）
- `warning`：排除部分周期时的说明

部分周期仍包含在`epoch_apy_data`中，并使用`is_partial: true`以保持透明性，但它们不会影响整体APY计算。
</Note>
...
<CardGroup cols={2}>
  <Card title="总APY" icon="coins">
    **验证者总APY**：所有奖励除以质押量，未扣除任何佣金。这代表了验证者的全部收益潜力。
  </Card>
  <Card title="质押者APY" icon="hand-holding-dollar">
    **扣除佣金后**：质押者/委托者在扣除所有奖励类型的佣金费用后实际收到的APY。
  </Card>
</CardGroup>

总APY与质押者APY之间的差异反映了验证者在不同奖励类型（即，通胀、MEV、优先费用）中的佣金结构。
...
| 组件 | 描述 |
|-----------|-------------|
| **通胀APY** | Solana的通胀奖励分配给验证者的回报 |
| **MEV APY** | 来自Jito MEV提示的最佳区块排序回报 |
| **区块奖励APY** | 交易发送者支付的优先费用回报 |

**总APY** = 通胀APY + MEV APY + 区块奖励APY
...
3. 计算期间所有epoch的平均APY
4. 应用佣金扣除以计算质押者APY（即委托者实际收到的APY）

与简单线性外推相比，这种方法在较短时间内提供了更准确的APY估算。
