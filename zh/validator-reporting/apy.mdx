---
title: 获取验证者 APY
description: 计算验证者的年化收益率，包括总 APY（验证者的总收益）和委托者 APY（扣除佣金后委托者实际收到的收益）
---
...
返回 **总 APY**（扣除佣金前的验证者总收益）和 **委托者 APY**（扣除通胀、MEV 和优先费用后的委托者实际收到的 APY），并按奖励类型提供详细的分解和可选的预测。
...
<CardGroup cols={2}>
  <Card title="总 APY" icon="coins">
    **验证者总 APY**：所有奖励除以质押量，未扣除任何佣金。这代表了验证者的全部收益潜力。
  </Card>
