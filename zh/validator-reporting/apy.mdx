---
title: 获取验证者 APY
description: 通过总 APY（验证者总收益）和质押者 APY（扣除佣金后委托人收到的收益）计算验证者的年化收益率
---
...
返回 **总 APY**（扣除佣金前的验证者总收益）和 **质押者 APY**（扣除通胀、MEV 和优先费用后的实际 APY），并按奖励类型提供详细的分解和可选的预测。
...
在计算 APY 时，API 会自动排除部分周期（即，具有不完整质押奖励数据的周期）以确保计算准确。

<Note>
**工作原理：**
- `epochs_in_range`：您请求范围内的总周期数（例如，3 个周期）
- `epochs_covered`：用于 APY 计算的完整周期数（例如，2 个周期）
- `epochs_partial`：排除的部分周期数（例如，1 个周期）
- `warning`：排除部分周期时的说明

部分周期仍包含在 `epoch_apy_data` 中，并使用 `is_partial: true` 以保持透明性，但它们不影响整体 APY 计算。
</Note>
...
总 APY 与质押者 APY 之间的差异反映了验证者在不同奖励类型（即，通胀、MEV、优先费用）中的佣金结构。
...
3. 计算期间所有epoch的平均APY
4. 应用佣金扣除以计算质押者APY（即委托人实际收到的APY）
