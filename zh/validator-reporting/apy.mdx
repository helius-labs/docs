---
title: 获取验证者 APY
description: 计算验证者的年化收益率，包括总 APY（验证者的总收益）和委托人 APY（扣除佣金后委托人收到的收益）
---
...
通过在指定期间内平均每个 epoch 的 APY 来计算验证者的年化收益率。

返回 **总 APY**（扣除佣金前的验证者总收益）和 **委托人 APY**（扣除通胀、MEV 和优先费用后的实际 APY），并按奖励类型提供详细的分解和可选的预测。

请注意，优先费用（即区块奖励）目前归属于验证者，因为 SIMD-123 尚未上线，协议内没有与委托人共享的方式。
...
在计算 APY 时，API 会自动排除部分周期（即，具有不完整质押奖励数据的周期）以确保计算的准确性。

<Note>
**工作原理：**
- `epochs_in_range`：您请求范围内的总周期数（例如，3 个周期）
- `epochs_covered`：用于 APY 计算的完整周期数（例如，2 个周期）
- `epochs_partial`：被排除的部分周期数（例如，1 个周期）
- `warning`：排除部分周期时的解释

部分周期仍包含在 `epoch_apy_data` 中，并使用 `is_partial: true` 以保持透明性，但它们不会影响整体 APY 计算。
</Note>
...
| 组成部分 | 描述 |
|-----------|-------------|
| **通胀 APY** | 来自 Solana 通胀奖励分配给验证者的回报 |
| **MEV APY** | 来自 Jito MEV 提示的最佳区块排序回报 |
| **区块奖励 APY** | 来自交易发送者支付的优先费用的回报 |
...
1. 计算每个 epoch 的回报：`epoch_return = rewards / stake`
2. 使用每年 epoch 数进行年化：`APY = ((1 + epoch_return)^num_epochs_per_year - 1) * 100`
3. 计算期间所有 epoch 的平均 APY
4. 应用佣金扣除以计算质押者 APY（即委托人实际收到的 APY）

与简单线性外推相比，这种方法在较短时间内提供了更准确的 APY 估计。

---

## 相关端点

<Card title="Earnings" icon="money-bill-trend-up" href="/zh/validator-reporting/earnings">
  获取详细的收益分解以支持 APY 计算
</Card>

<Card title="Rewards Forecast" icon="chart-line" href="/zh/validator-reporting/rewards-forecast">
  使用多种模型和置信区间进行高级预测
</Card>
