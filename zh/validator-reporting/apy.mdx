...
在计算 APY 时，API 会自动排除部分 epoch（即，具有不完整质押奖励数据的 epoch），以确保计算准确。
...
部分 epoch 仍然包含在 `epoch_apy_data` 中，并使用 `is_partial: true` 以保持透明，但它们不会影响整体 APY 计算。
</Note>
