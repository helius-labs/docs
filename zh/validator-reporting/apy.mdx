---
title: 获取验证者 APY
description: 计算验证者的年收益率，包括总 APY（验证者的总收益）和委托人 APY（扣除佣金后委托人收到的收益）
---
...
请注意，优先费用（即区块奖励）目前归于验证者，因为 SIMD-123 尚未上线，协议内没有与委托人共享的方式。
...
- `epochs_partial`：被排除的部分 epoch 数（例如，1 个 epoch）
- `warning`：排除部分 epoch 时的解释

部分 epoch 仍然包含在 `epoch_apy_data` 中，并使用 `is_partial: true` 以保持透明性，但它们不会影响整体 APY 计算。
</Note>
...
<CardGroup cols={2}>
  <Card title="总 APY" icon="coins">
    **总验证者 APY**：所有奖励除以质押，在扣除任何佣金之前。这代表了验证者的全部收益潜力。
  </Card>
  <Card title="质押者 APY" icon="hand-holding-dollar">
    **扣除佣金后**：质押者/委托者在扣除所有佣金费用后实际收到的 APY。
  </Card>
</CardGroup>
...
| 组成部分 | 描述 |
|-----------|-------------|
| **通胀 APY** | 来自 Solana 通胀奖励分配给验证者的收益 |
| **MEV APY** | 来自 Jito MEV 提示的最佳区块排序收益 |
| **区块奖励 APY** | 来自交易发送者支付的优先费用的收益 |
