---
title: 获取验证者 APY
description: 计算验证者的年化收益率，包括总 APY（验证者的总收益）和委托者 APY（扣除佣金后委托者收到的收益）
---
...
在计算 APY 时，API 会自动排除部分 epoch（即，质押奖励数据不完整的 epoch），以确保计算准确。
...
部分 epoch 仍包含在 `epoch_apy_data` 中，并使用 `is_partial: true` 以保持透明性，但它们不影响整体 APY 计算。
</Note>
...
与简单线性外推相比，这种方法在较短时间内提供了更准确的APY估计。
