...
类型过滤在运行时发生，这意味着 API 会顺序搜索交易，直到找到至少 50 个匹配项。如果 API 在搜索期间找不到任何符合您过滤条件的交易，它将返回一个错误，并附有继续搜索的说明。
</Warning>
...
<Info>
**关键点**：
- 使用类型过滤器时，API 每次搜索最多可处理 50 笔交易
- 如果没有找到匹配项，请使用错误消息中的签名继续搜索
- 在降序（默认，最新优先）搜索时使用 `before-signature` 参数
- **在升序（最旧优先）搜索时使用 `after-signature` 参数** - 这是按时间顺序搜索所需的
- 实施最大重试限制以防止无限循环
