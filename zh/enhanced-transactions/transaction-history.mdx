...
- 结合使用`limit`参数来控制页面大小
- 当您希望按时间顺序处理交易时，使用`sort-order=asc`
- 基于时间的过滤器对日期范围更直观，而基于槽的过滤器对区块链特定查询更有用
</Tip>
...
类型过滤在运行时发生，这意味着API会顺序搜索交易，直到找到至少50个匹配项。如果API在搜索期间找不到任何匹配您的过滤条件的交易，它将返回一个错误，并附有继续搜索的说明。
</Warning>

使用类型过滤时，您可能会遇到在当前搜索窗口内找不到匹配交易的情况。在这种情况下，API会返回一个错误响应，如：
...
<Info>
**关键点**：
- 使用类型过滤器时，API一次最多搜索50个交易
- 如果没有找到匹配项，请使用错误消息中的签名继续搜索
- 在降序搜索时使用`before-signature`参数（默认，最新优先）
- **在升序搜索时使用`after-signature`参数（最旧优先）** - 这是按时间顺序搜索所必需的
- 实施最大重试限制以防止无限循环
- 这种行为是预期的，允许您搜索一个地址的整个历史记录以查找特定的交易类型
</Info>

## 有问题吗？

有关增强交易的常见问题，包括使用、身份验证、速率限制和故障排除，请访问我们全面的[增强交易常见问题](/zh/faqs/enhanced-transactions)。
