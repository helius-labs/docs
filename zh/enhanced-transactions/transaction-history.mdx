...
- 使用 `sort-order=asc` 进行升序（最早的优先）或 `sort-order=desc` 进行降序（最新的优先）
</Info>
...
- 基于时间的过滤器对于日期范围更直观，而基于槽的过滤器对于区块链特定查询更有用
</Tip>
...
类型过滤在运行时发生，这意味着API会顺序搜索交易，直到找到至少50个匹配项。如果API在搜索期间找不到任何匹配您过滤条件的交易，它将返回一个错误，并附有继续搜索的说明。
</Warning>

使用类型过滤器时，您可能会遇到在当前搜索窗口内找不到匹配交易的情况。在这种情况下，API会返回一个错误响应，如：
...
- **搜索升序时使用`after-signature`参数（最旧优先）** - 这是按时间顺序搜索所需的
- 实施最大重试限制以防止无限循环
- 这种行为是预期的，允许您搜索一个地址的整个历史记录以查找特定交易类型
</Info>
