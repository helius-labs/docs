...
title: 解析交易
sidebarTitle: 解析交易
description: 将交易解析成人类可读的数据。
---

<Warning>
**重要限制**：
- 在我们开发V2期间，增强型交易API V1将不会更新
- 我们仅解析与NFT、Jupiter和SPL相关的交易
- **不要依赖这些解析器用于DeFi或非NFT、Jupiter和SPL交易**
</Warning>

## 概述

解析交易端点将原始交易签名或数据转换为结构化的、可读的信息。您无需手动解码指令数据和账户列表，即可获得关于转账、交换、NFT活动等的清晰细节。

<Card title="API参考" horizontal icon="code" href="/zh/api-reference/enhanced-transactions/gettransactions">
  查看详细的解析交易API文档
</Card>

## 快速开始

通过单个API调用解析一个或多个交易签名：
...
有关增强型交易的常见问题，包括使用、认证、速率限制和故障排除，请访问我们全面的[增强型交易FAQ](/zh/faqs/enhanced-transactions)。
