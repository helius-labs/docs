...
transactionSubscribe websocket 方法启用实时交易事件。要使用它，请提供 `TransactionSubscribeFilter`，并可选择性地包括 `TransactionSubscribeOptions` 以进行进一步的自定义。
...
`accountInclude`：要接收交易更新的账户列表。这意味着交易更新中必须包含其中一个账户（例如，账户1或账户2）。

`accountExclude`：要从交易更新中排除的账户列表。

`accountRequired`：交易必须涉及这些指定的账户才能包含在更新中。这意味着交易更新中必须包含所有账户（例如，账户1和账户2）。
...
`maxSupportedTransactionVersion`：指定要接收更新的最高版本交易。要获取遗留和v0交易，请将值设置为 `0`。
...
在此示例中，我们订阅包含 Raydium 账户 `675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8` 的交易。每当发生包含 `675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8` 的交易时，我们将在交易的 `accountKeys` 中收到 websocket 通知。
