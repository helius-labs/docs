...
transactionSubscribe websocket 方法启用实时交易事件。要使用它，请提供一个 `TransactionSubscribeFilter`，并可选择性地包括 `TransactionSubscribeOptions` 以进行进一步的自定义。
...
`accountInclude`：要接收交易更新的账户列表。这意味着交易更新中只需包含其中一个账户（例如，账户1或账户2）。
...
`maxSupportedTransactionVersion`：指定要接收更新的最高版本交易。要获取传统和 v0 交易，请将值设置为 `0`。
