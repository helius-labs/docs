...
* `encoding`：指定在AccountNotification中返回数据的格式。支持的值：**base58**、**base64**、**base64+zstd**、**jsonParsed**（默认是**base58**）。
...
在此示例中，我们订阅账户 `SysvarC1ock11111111111111111111111111111111` 的更改。 \
\
每当此账户的数据或lamports发生变化时，我们将看到更新。\
由于 `slot` 和 `unixTimestamp` 都是返回账户数据的一部分，因此此特定账户会在频繁的间隔内发生这种情况。
