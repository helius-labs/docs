...
401错误表示身份验证问题。检查您的API密钥是否正确，是否正确包含在您的请求中（如`?api-key=YOUR_API_KEY`），并确保您的账户有权访问您尝试使用的端点。
</Accordion>

<Accordion title="为什么我会收到429错误？">
429错误表示您已超出速率限制。查看您的[计划和速率限制](https://www.helius.dev/docs/billing/plans-and-rate-limits)，考虑升级您的计划或在应用程序中实施请求限制。
</Accordion>
