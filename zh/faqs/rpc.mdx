...
`getProgramAccounts` 是一个资源密集型方法，除了标准的每秒请求数限制外，还有额外的速率限制。请考虑使用过滤参数，或将大型查询分解为较小的部分以保持在[限制](/zh/billing/rate-limits)内。
</Accordion>
...
<Accordion title="为什么我的sendTransaction请求没有成功？">
调用**Priority Fee API**，优化计算单元的使用，并使用`skipPreflight: true`发送交易。请参阅我们的[**交易优化指南**](https://www.helius.dev/docs/sending-transactions/optimizing-transactions)以获取分步演练。
</Accordion>
...
<Accordion title="获取最新blockhash的最佳实践是什么？">
使用`getLatestBlockhash`和`"confirmed"`承诺级别来获取最新的blockhash。在提交交易之前，始终获取新的blockhash以确保有效性。详情请参阅我们的[getLatestBlockhash指南](/zh/rpc/guides/getlatestblockhash)。
</Accordion>
