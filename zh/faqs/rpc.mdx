...
`getProgramAccounts` 是一种资源密集型方法，除了标准的每秒请求数限制外，还有额外的速率限制。请考虑使用过滤参数，或将大型查询分解为较小的部分以保持在[限制](/zh/billing/rate-limits)范围内。
</Accordion>
...
使用 `getLatestBlockhash` 和 `"confirmed"` 承诺级别来获取最新的 blockhash。在提交交易之前始终获取新的 blockhash 以确保有效性。有关详细信息，请参阅我们的 [getLatestBlockhash 指南](/zh/rpc/guides/getlatestblockhash)。
</Accordion>
