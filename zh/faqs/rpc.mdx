...
<Accordion title="为什么在计划的速率限制下，我的getProgramAccounts会被限速？">
`getProgramAccounts` 是一个资源密集型方法，除了标准的每秒请求数限制外，还有额外的速率限制。请考虑使用过滤参数，或将大型查询分解为较小的部分，以保持在[限制](/zh/billing/rate-limits)范围内。
</Accordion>
...
由于计算要求，一些方法如getProgramAccounts有特殊的速率限制。然而，它们仍然计入总的RPC速率限制。详情请阅读我们的[特殊速率限制指南](https://www.helius.dev/docs/billing/rate-limits)。
</Accordion>
