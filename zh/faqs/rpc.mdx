...
`getProgramAccounts` 是一个资源密集型方法，除了标准的每秒请求数之外，还有额外的速率限制。请考虑过滤参数，或将大型查询分解为较小的块以保持在[限制](/zh/billing/plans-and-rate-limits#high-complexity-calls)内。
</Accordion>
...
<Accordion title="获取最新区块哈希的最佳实践是什么？">
使用`getLatestBlockhash`和`"confirmed"`承诺级别来获取最新的区块哈希。在提交交易之前，始终获取一个新的区块哈希以确保有效性。详情请参阅我们的[getLatestBlockhash 指南](/zh/rpc/guides/getlatestblockhash)。
</Accordion>
...
您的RPC速率限制适用于所有RPC方法的总和，而不是单独适用于每个方法。例如，如果您的RPC速率限制是100，并且您每秒进行50次getAccountInfo请求，那么您只能额外进行50次getTransaction请求。
</Accordion>

<Accordion title="特殊限制如何影响RPC速率限制？">
由于计算要求，一些方法如getProgramAccounts有特殊的速率限制。然而，它们仍然计入总的RPC速率限制。详情请阅读我们的[特殊速率限制指南](https://www.helius.dev/docs/billing/plans-and-rate-limits#special-limits)。
</Accordion>

<Accordion title="RPC速率限制与DAS速率限制之间的关系是什么？">
