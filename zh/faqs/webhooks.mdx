...
您的端点应返回200状态码以确认成功接收。如果您的端点返回错误状态码或根本没有响应，我们将重试传递最多3次，每次尝试之间间隔1分钟。如果所有重试尝试都失败，webhook事件将被丢弃。
...
`TOKEN_MINT` webhook 仅针对特定程序触发，如 Candy Machine V1 和 Solana Program Library。它不涵盖整个区块链上的所有代币铸造事件。对于更广泛的监控，考虑通过 webhooks 使用 "ANY" 交易类型过滤来订阅更通用的账户或程序。
</Accordion>
