...
在创建或更新 webhook 时设置一个授权头。Helius 在向您的 webhook 端点发送数据时，会在 `Authorization` 头中回显此值，从而允许您验证请求的真实性。
</Accordion>
...
`TOKEN_MINT` webhook 仅针对特定程序触发，如 Candy Machine V1 和 Solana Program Library。它不涵盖整个区块链上的所有代币铸造事件。对于更广泛的监控，请考虑通过 webhook 使用 "ANY" 交易类型过滤进行更通用的账户或程序订阅。
</Accordion>
