...
<CardGroup cols={2}>
  <Card title="查询NFTs" icon="image" href="#working-with-nfts-and-digital-collectibles">
    检索、搜索和管理NFT数据和收藏
  </Card>
  <Card title="访问SPL代币" icon="coins" href="#working-with-spl-tokens">
    获取代币余额、账户和持有人信息
  </Card>
  <Card title="代币定价" icon="chart-line" href="#price-data-for-tokens">
    获取前10k代币按24小时交易量的实时价格数据
  </Card>
  <Card title="API参考" icon="code" href="/zh/api-reference/das">
    查看详细的API文档
  </Card>
</CardGroup>
...
<Warning>
getAsset 返回的价格数据是缓存的，可能不是最新的。价格信息有一个 600 秒的缓存，这意味着数据可能最多有 600 秒的延迟。
</Warning>
...
1. 对于返回大量数据的方法，**使用分页**
2. 通过实现try/catch块，**优雅地处理错误**
3. 在适当的时候**缓存响应**以减少API调用
4. **遵守速率限制**以避免应用程序中断
5. 在计算市值之前，**验证价格数据**是否可用
