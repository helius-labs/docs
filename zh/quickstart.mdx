...
<Steps titleSize="h2"> 
  <Step title="创建您的免费 Helius 账户">
    首先在 [Helius Dashboard](https://dashboard.helius.dev/dashboard) 创建您的免费账户。您的免费套餐每月包括 100,000 次 DAS API 调用，非常适合入门和构建原型。
  </Step>
  <Step title="获取您的 API 密钥">
    导航到 [API Keys](https://dashboard.helius.dev/api-keys) 部分并复制您的密钥。此密钥为您提供访问所有 Helius API 的权限，包括 RPC 节点、DAS API 和增强的交易数据。
  </Step>
  <Step title="进行您的第一次 API 调用">
    让我们从一个实际示例开始：从钱包中获取 NFT。我们将使用 [getAssetsByOwner](/zh/api-reference/das/getassetsbyowner) 方法查询 `86xCn…o2MMY`（Solana 联合创始人 Anatoly Yakovenko 的钱包）拥有的资产。
    
    <Quickstart />
    
    **这里发生了什么？** DAS API 允许您通过一次调用查询压缩和标准 NFT。请注意，响应中包括元数据、图像 URL 和所有权信息——构建丰富用户体验所需的所有数据。
  </Step>
  <Step title="理解响应">
...
<CardGroup cols={2}>
  ...
  <Card title="高级 NFT 操作" icon="images" href="/zh/das/get-nfts">
    按特征搜索 NFT，获取集合统计信息，并大规模处理代币元数据。
  </Card>
  <Card title="实时数据流" icon="bolt" href="/zh/data-streaming/quickstart">
    使用 gRPC 或增强的 WebSockets 以亚秒级延迟流式传输实时区块链数据。
  </Card>
  <Card title="使用 Webhooks 自动化" icon="bell" href="/zh/webhooks">
