...
<Steps titleSize="h2"> 
  ...
  <Step title="获取您的 API 密钥">
    导航到 [API Keys](https://dashboard.helius.dev/api-keys) 部分并复制您的密钥。此密钥使您可以访问所有 Helius API，包括 RPC 节点、DAS API 和增强的交易数据。
  </Step>
  <Step title="进行您的第一次 API 调用">
    让我们从一个实际的例子开始：从钱包中获取 NFT。我们将使用 [getAssetsByOwner](/zh/api-reference/das/getassetsbyowner) 方法查询 `86xCn…o2MMY`（Solana 联合创始人 Anatoly Yakovenko 的钱包）拥有的资产。
