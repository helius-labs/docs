...
您可以通过选择客户端类型来自定义您的节点——可以是 Agave 或 Jito Labs（Agave 的分支，具有额外的方法 `simulateBundle`）
...
在这里，我们使用 Solana web3.js 通过我们的专用节点调用 [`getSlot`](/zh/api-reference/rpc/http/getslot)：
...
- 不支持归档数据查询（使用您的共享计划）
- 支持 `getProgramAccounts` 但未优化 - 大量使用可能会影响性能或导致节点故障（使用您的共享计划以确保可靠性）
