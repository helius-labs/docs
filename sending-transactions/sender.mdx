...
<Warning>
**Mandatory Requirements**: All transactions must include Jito tips (minimum 0.001 SOL) and skip preflight checks.
</Warning>
...
- **Jito Tips**: Minimum 0.001 SOL transfer to designated tip accounts (required for Jito auction)
...
**Jito Tips**: Required for participation in Jito's auction mechanism—see the [tip guidelines](https://docs.jito.wtf/lowlatencytxnsend/#tips) for details.

**Priority Fees**: Increase your transaction's priority in Solana's scheduling. While not strictly required by the protocol, they significantly speed up confirmation during network congestion.

#### How Tips and Priority Fees Work
...
**Jito Tips** (see [tip recommendations](https://docs.jito.wtf/lowlatencytxnsend/#tips)):
- **Minimum**: 0.001 SOL (required for auction participation)
- **Standard**: 0.001-0.005 SOL depending on network activity
- **High Priority**: 0.01+ SOL during peak congestion

**Priority Fee Recommendations**:
- **Low Congestion**: 10,000-50,000 microLamports per CU
- **Medium Congestion**: 50,000-200,000 microLamports per CU  
- **High Congestion**: 200,000+ microLamports per CU

Need a dynamic estimate instead of hard-coding numbers? Check out the [Helius Priority Fee API](/priority-fee) for real-time recommendations.
...
<Note>
**Important**: Jito tips are separate SOL transfer instructions, while priority fees are set via `ComputeBudgetProgram.setComputeUnitPrice`. Both are required for Sender's dual routing system; tips for Jito auction access and priority fees to boost validator prioritization.
</Note>
