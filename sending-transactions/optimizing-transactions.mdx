...
- Optimize [compute unit](/rpc/guides/getfeeformessage) (CU) usage
...
Both the Helius [Node.js](/sdks) and [Rust](/sdks) SDKs can send smart transactions. This new method builds and sends an optimized transaction while handling its confirmation status. Users can configure the [transaction's send options](/rpc/guides/sendtransaction), such as whether the transaction should skip preflight checks.
