...
sidebarTitle: "Jupiter Swap API via Sender"
...
<ol>
  <li>Get a [Jupiter quote](https://dev.jup.ag/docs/api/swap-api/quote) for a trading pair</li>
...
Right now, the tip amount is hardcoded at the minimum 0.001 SOL. However, this may not be enough to win the Jito auction for highly contested accounts. For these cases, use the [Jito API](https://docs.jito.wtf/lowlatencytxnsend/#tip-amount) to dynamically compute the Sender tip:
