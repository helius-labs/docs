...
<Steps>
  ...
  <Step title="Configuration">
    Replace the hardcoded API key and private key with your credentials. Set the right constants for your swap: token addresses, amount, slippage. Choose the Sender enpoint that's closest to your server (it's currently EWR).
  </Step>
...
### Dynamic tipping with the Jito API

In the code above, the tip amount is hardcoded at the minimal 0.001 SOL. This may not be enough to win the Jito auction for highly contested accounts. To solve this, use the Jito API to dynamically compute the Sender tip:
