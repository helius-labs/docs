...
Returns both **Gross APY** (total validator earnings before commissions) and **Staker APY** (the actual APY that stakers/delegators receive after all commission deductions across inflation, MEV, and priority fees), with detailed breakdowns by reward type and optional forecasting. Priority fees (i.e., block rewards) are currently attributed to the validator, since SIMD-123 is not live yet and there is no in-protocol way to share with delegators.
...
<ResponseField name="calculation_basis" type="object">
  ...
  <Expandable title="calculation_basis properties">
    ...
    <ResponseField name="calculation_method" type="string">
      Method used for APY calculation (i.e., the same methodology as APY queries on Dune)
    </ResponseField>
  </Expandable>
</ResponseField>
